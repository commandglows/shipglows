...
evidence:
  ...
  - "Verified Core delivery 2026-08-28 adds one always-on required CI gate, guarded install-before-protect reconciliation, and project-by-project provider proof for managed GitHub repositories."
  ...
  - "The Windows managed developer-tool update implementation and static contracts support bounded future-facing copy; installed-host and release proof remain mandatory before availability claims are published."
...
| Native context intelligence | ShipGlows can say it prepares a local, fresh, bounded and explainable task context for material work. It must identify the graph and capsule as derived and advisory, preserve visible gaps and truncation, and attribute measurements to their tested repositories and tasks. | `shipglows_data/workflow/specs/industrialize-native-context-intelligence.md`, `shipglows_data/technical/native-code-context-graph.md`, PR #47 | `allowed with caveat` | Landing, blog, docs, README | Block perfect understanding, universal language coverage, zero omissions, hidden uncertainty, external-service implications, or unmeasured productivity claims |
...
| Git persistence, recovery, and managed CI protection | ShipGlows can say its governed workflow requires exact-scope commits and remote pushes at validated milestones and clean closure, checks for locally vulnerable work at existing lifecycle boundaries, and distinguishes local, backed-up, CI-protected, and deployed states. For active managed GitHub projects, it can say the Core defines and audits one always-on `ShipGlows required gate`, generates relevant stack lanes with explicit successful no-impact results, and refuses to enable repository protection before the exact workflow and provider evidence prove the check. It must not guarantee GitHub availability, zero data loss, every CI run, blanket repository protection, completed fleet rollout, or deployment from a push alone. | `skills/references/git-milestone-delivery-contract.md`, `skills/references/git-persistence-preflight.md`, `skills/references/managed-project-ci-policy.md`, `shipglows_data/workflow/specs/managed-project-required-ci-gate.md`, focused contract tests | `allowed with caveat` | Landing, docs, FAQ, README, articles | Block guarantees, provider-state assumptions, unverified project-compliance claims, or wording that treats Core capability, remote backup, CI protection, and deployment as interchangeable |
| Managed Windows developer-tool maintenance | ShipGlows can describe the implemented separation between ShipGlows self-update, read-only global tool status, confirmed allowlisted global updates, and project dependency ownership. Exact-ID WinGet and exact-version npm/pnpm behavior may be explained as prepared functionality, but stable availability must not be claimed before installed-host convergence and release delivery are proven. | `shipglows_data/workflow/specs/windows-managed-developer-tool-updates.md`, commits `11363b6` and `69d5bf2`, focused and complete Windows static contracts | `needs proof` for availability; `allowed with caveat` for draft explanation | Runtime install pages, docs, changelog, EN/FR article pair | Block publication while article peers are not draft, wording omits the future-release caveat, installed-host proof is missing, or a surface implies broad system/project updates |
...
### Windows developer-tool maintenance — 2026-08-30

- Claim: ShipGlows separates its own updates, global Windows developer-tool maintenance, and project dependencies; the global path previews first and updates only an explicit allowlist after confirmation.
- Claim family: automation and supply-chain-sensitive local behavior.
- Affected surfaces: EN/FR runtime install data, EN/FR docs sections, EN/FR indexed article drafts, repository changelog.
- Evidence: ready spec, implementation commits `11363b6` and `69d5bf2`, focused Windows update contract, complete Windows static contract.
- Status: `needs proof` for availability; `allowed with caveat` for draft explanation.
- Allowed wording: describe the bounded implementation and label it as prepared for the next Windows runtime release.
- Required action: keep articles in draft and do not deploy or announce availability before installed-host convergence and release proof.
- Stop condition: any public stable-availability claim before the real Windows update path and delivered release are verified.
