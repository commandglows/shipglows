  ...
  sg-explore -> sg-spec -> revue adversariale de la spec -> sg-start ->
  implémentation -> sg-verify -> sg-end

  Le point clé : sg-start ne devrait plus être le moment où on découvre le
  problème. Il devrait être le moment où on engage une implémentation déjà
  clarifiée.

  Ce que je changerais dans le flow

  1. sg-explore devient l’étape optionnelle de désembrouillage.
  ...
  2. sg-spec devient la porte d’entrée par défaut pour tout scope medium+.
     Pas juste une doc technique, mais un contrat d’implémentation.
     Si la spec n’est pas autonome, on ne code pas.
  3. sg-start devient un “execution kickoff”.
  ...
  4. sg-verify vérifie contre la spec, pas seulement contre le diff.
  ...
  5. sg-end doit fermer la boucle avec traçabilité.
     Il devrait résumer :
