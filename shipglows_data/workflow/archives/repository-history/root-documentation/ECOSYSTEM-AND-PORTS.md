...
supersedes: []
evidence:
  - "Document title and body describe ShipGlows automatic port allocation and PM2 ecosystem behavior"
...
ShipGlows gère automatiquement les ports et crée des fichiers `ecosystem.config.cjs` persistants pour chaque projet, garantissant que les tunnels SSH locaux détectent tous les projets.
...
Lors du démarrage d'un projet avec ShipGlows, un fichier `ecosystem.config.cjs` est automatiquement créé :
...
Quand vous démarrez un nouveau projet, ShipGlows :
...
Une fois qu'un port est assigné à un projet (soit manuellement, soit via l'allocation automatique), ShipGlows le réutilisera pour ce projet lors des démarrages futurs. Le port est stocké dans le fichier `ecosystem.config.cjs` du projet.
