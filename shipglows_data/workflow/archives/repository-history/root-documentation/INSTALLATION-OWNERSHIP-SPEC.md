...
linked_systems:
  ...
  - dotfiles/lib.sh
evidence:
  - "shipglows/install.sh installe aujourd'hui des dependances systeme et configure root + tous les comptes /home, mais n'installe pas explicitement Claude Code ni OpenAI Codex."
  ...
  - "User request: dotfiles should focus on generic tooling; ShipGlows should own AI/code/developpement actif responsibilities."
next_step: "Aligner les scripts et README sur cette spec avant implementation."
---

# Spec: Ownership d'installation entre dotfiles et ShipGlows

## Title

Separation des responsabilites d'installation entre `dotfiles` et `ShipGlows`
...
- `ShipGlows` configure les espaces Claude/Codex/skills/MCP, installe les CLIs IA/code de son workflow via `pnpm`, et doit maintenant permettre une sélection fine par agent au lieu d'imposer un bundle unique.
...
`ShipGlows` doit etre le bootstrap IA/code/developpement actif. Il possede les assistants IA, les MCP, les skills, les conventions Claude/Codex, et les donnees de travail ShipGlows.
...
`ShipGlows` must not own:
- generic terminal/editor dotfiles.
- OS-level bootstrap unrelated to AI/code workflow, except where required by ShipGlows runtime.
...
- `ShipGlows` should not independently create users by default; it should detect, explain, and either reuse the selected user or defer to the bootstrap path.
- If ShipGlows does gain user creation later, it must use the same contract and not duplicate dotfiles behavior.
...
- Stops running ShipGlows installer automatically unless explicitly requested.
...
Already aligned:
- Owns Claude/Codex config, skills, MCP, statusline, ShipGlows aliases, `shipglows_data`.
...
- Moves generic tooling responsibilities out of ShipGlows unless required by ShipGlows runtime.
...
2. `ShipGlows` configures AI/code workflow for the selected operational user.
...
2. `ShipGlows` updates AI/code config for the current or explicitly selected user.
...
- [ ] AC1: Documentation states that `dotfiles` owns generic tooling and `ShipGlows` owns AI/code workflow.
...
- [ ] AC5: `dotfiles` no longer silently owns Claude/Codex/MCP configuration once ShipGlows owns that domain.
- [ ] AC6: ShipGlows either installs or validates the selected AI CLIs (`claude`, `codex`, `opencode`, `kilocode`) and explains missing binaries explicitly.
...
- Keep `specs/install-user-targeting.md` as the ShipGlows-specific implementation spec until merged or superseded.
- Before code changes, decide whether AI CLI binary installation moves from `dotfiles` to `ShipGlows`, or whether `ShipGlows` only validates existing binaries.
...
- Should ShipGlows keep following latest npm releases for selected agents, or expose a pinned/stable channel later?
- Should `dotfiles` be allowed to call `shipglows/install.sh`, or should it only print the next command?
