...
- [x] AC 31: Given an interactive native Windows update, when a provider command runs or user input is required, then the current phase/input/answer-received state is visible and phase work duration excludes human wait time; an ambiguous provider exit never overrides stronger bounded final evidence from a trusted executable with an autonomous calendar version, exact service CLI, native Claude re-read, decoded Codex MCP JSON, or a healthy Flutter doctor report without `[!]`/`[X]` Android failure markers; generated Rust wrappers reproduce the same safe isolated mise environment without mutating global trust and preserve command arguments and exit status.
