...
scope: migration
owner: Diane
user_story: "En tant qu'utilisatrice ShipGlows qui pilote plusieurs projets avec des agents, je veux que tous les documents Markdown ShipGlows d'un projet vivent sous un dossier shipglows_data versionne dans le repo du projet, afin d'arreter les fichiers eparpilles, les symlinks absolus casses et les doctrines documentaires contradictoires."
confidence: high
risk_level: high
security_impact: yes
docs_impact: yes
...
evidence:
  ...
  - "User decision 2026-05-09: all project ShipGlows Markdown categories belong under the project-local shipglows_data folder, including workflow artifacts."
  - "User decision 2026-05-09: project-level folders are the only source of truth for this phase; the central shipglows_data repo and master trackers are out of scope for now."
  ...
  - "Local audit 2026-05-09: socialflow/TASKS.md, tubeflow_expo/TASKS.md, and tubeflow_flutter/tubeflow_lab/TASKS.md are versioned or local symlinks pointing into shipglows_data."
  ...
  - "Existing doctrine before this spec: project decision docs live in project repos, while shipglows_data is limited to trackers and registries."
  ...
  - "User decision 2026-05-10: workflow artifacts use shipglows_data/workflow/specs, bugs, audits, reviews, and verification."
  - "User decision 2026-05-10: project trackers live under the project corpus; master trackers remain in the central shipglows_data repo and are ignored for now."
...
next_step: "/sg-start ShipGlows Data Project Umbrella Documentation Architecture"
---

# ShipGlows Data Project Umbrella Documentation Architecture
...
The project-level doctrine is locked and ready for `/sg-start`. Every project repo owns a `shipglows_data/` folder at its Git root; all project-owned ShipGlows Markdown lives under that folder; project-level folders are the source of truth for this phase. The central `shipglows_data` repo, master trackers, and any future sync mechanism are deliberately out of scope for now.

## Recorded Doctrine

- Each GitHub project gets a `shipglows_data/` folder at the repository root.
- The project-local `shipglows_data/` folder is the source of truth for that project's ShipGlows Markdown.
- All project-owned ShipGlows Markdown goes under that folder, including technical, editorial, business, workflow, specs, bugs, audits, reviews, verification reports, and project-level trackers.
- Master trackers live in the separate `shipglows_data` repository root and are meant to be an aggregate/fusion layer, but they are ignored for this architecture phase.
- Do not design this phase around the central `shipglows_data` repository. Only project-local folders are read as sources of truth.
- If a future master synchronization exists, the direction is project repo -> master. Whether that is implemented by symlink, copy, generated index, or the ShipGlows web app is intentionally undecided.
- Versioned project files must not point to `/home/ubuntu`, `/home/claude`, or any other user-specific absolute path.
- Every skill that creates, reads, updates, verifies, ships, audits, or routes project-owned ShipGlows Markdown must respect the project-local `shipglows_data/` doctrine.
- If a skill detects missing, scattered, legacy-root, duplicated, or otherwise non-compliant ShipGlows Markdown, it must not keep writing into the old structure. It must route to the document owner skill for creation, update, or reorganization.
- `sg-init` owns initial creation of the project-local `shipglows_data/` corpus for new or adopted projects.
- `sg-docs` owns document updates, audits, and reorganization/migration of mislocated ShipGlows Markdown into the canonical project-local `shipglows_data/` layout.
...
En tant qu'utilisatrice ShipGlows qui pilote plusieurs projets avec des agents, je veux que tous les documents Markdown ShipGlows d'un projet vivent sous un dossier `shipglows_data/` versionne dans le repo du projet, afin d'arreter les fichiers eparpilles, les symlinks absolus casses et les doctrines documentaires contradictoires.

## Minimal Behavior Contract

Each project repository must expose one canonical project-local `shipglows_data/` umbrella at its Git root, organized by domain, and that folder is the only source of truth for project-owned ShipGlows Markdown during this phase. If a project has legacy root-level ShipGlows Markdown files or symlinks into a central data directory, migration classifies and moves or replaces them without losing content. If a file is required at repo root by agent tooling, it becomes a compatibility entrypoint and must point readers to the canonical project-local `shipglows_data/` content without becoming a second source of truth. Any skill that finds missing or non-compliant project documentation must stop before writing more drift, then route to the document creation/update owner: `sg-init` for first bootstrap and `sg-docs` for update, audit, and reorganization. The central `shipglows_data` repository and master trackers are deliberately ignored for now; future sync, if any, flows from project repos toward the master layer and is not part of this phase.

## Success Behavior

- Given a fresh agent enters any adopted project repo, when it looks for ShipGlows documentation, then it finds `shipglows_data/` at the Git root.
- Given a project has `BUSINESS.md`, `PRODUCT.md`, `BRANDING.md`, `GTM.md`, `CONTENT_MAP.md`, `docs/technical/`, `docs/editorial/`, specs, bugs, audit reports, trackers, or decision records, when migration runs, then each ShipGlows-owned Markdown file lands under project-local `shipglows_data/` with metadata preserved.
- Given a skill creates or updates project-owned ShipGlows Markdown, when it resolves the target path, then it writes under the project umbrella unless the file is a documented root compatibility entrypoint.
- Given any skill detects project-owned ShipGlows Markdown outside the canonical `shipglows_data/` layout, when the skill is not the document owner, then it routes to `sg-docs` instead of editing the non-compliant layout.
- Given a project has no project-local `shipglows_data/` corpus yet, when initialization or adoption runs, then `sg-init` creates the baseline layout or reports why it is blocked.
- Given a project has scattered or duplicated ShipGlows Markdown, when `sg-docs` runs in update/reorganize mode, then it moves or plans moves into canonical folders with conflict handling and validation.
- Given a skill needs master or cross-project data, when this phase is active, then it does not treat the central `shipglows_data` repository as a source of truth for project documentation.
- Given a symlink is committed to Git, when it is inspected, then it is relative and repo-local only, never an absolute path to a user home or global `shipglows_data`.

## Error Behavior

- If a versioned symlink points to `/home/<user>/shipglows_data`, migration must report it as non-portable and remove or replace it before shipping.
...
- If a skill reads project documentation from the central `shipglows_data` repo during this phase, verification reports a source-of-truth violation.
- If a non-owner skill attempts to create or update project-owned ShipGlows Markdown in a legacy root path, verification reports a doctrine violation and routes to `sg-docs`.
...
- If the central `shipglows_data` folder contains a copied project decision document that also exists in the project umbrella, verification reports the project-local copy as canonical and the central copy as out of scope until the future sync model is defined.
...
ShipGlows currently mixes several ownership models:
...
- Chantiers live under `specs/`, bugs under `shipglows_data/workflow/bugs/`, and trackers under `TASKS.md`, `AUDIT_LOG.md`, or `${SHIPGLOWS_DATA_DIR:-$HOME/shipglows_data}`.
- Older workflows created or preserved project `TASKS.md` symlinks pointing into `shipglows_data`.
- Some docs say project decision docs should stay in project repos and should not be duplicated into `shipglows_data`; the new requested architecture keeps that project ownership and groups those docs under one project-local `shipglows_data/` umbrella.
...
Adopt a project-local `shipglows_data/` documentation umbrella as the canonical home for all project-owned ShipGlows Markdown. Do not design the first migration around the central `shipglows_data` repository. The central master layer can later consume, copy, index, or link project-local folders, but that future sync model is explicitly deferred.
...
```text
${SHIPGLOWS_DATA_DIR:-$HOME/shipglows_data}/
├── TASKS.md
├── AUDIT_LOG.md
├── PROJECTS.md
└── projects/
...
- Update skills that create, read, or validate ShipGlows Markdown paths.
- Add verification for hardcoded user-home paths and versioned symlinks to `shipglows_data`.
- Reconcile older tracker doctrine in `sg-help`, `sg-tasks`, `sg-init`, runtime docs, README, and the metadata migration guide.

## Scope Out

- Moving application runtime docs that are not ShipGlows-owned, such as framework docs consumed by the app, public docs pages, MDX/blog content parsed by the app, or package README files.
- Rewriting all historical research notes unless they are promoted into the active ShipGlows corpus.
- Converting operational trackers into decision artifacts with frontmatter.
- Replacing Git, GitHub, or cloud synchronization.
- Designing or implementing central `shipglows_data` repo synchronization.
...
- Root `BUSINESS.md`, `PRODUCT.md`, `BRANDING.md`, `GTM.md`, `CONTENT_MAP.md`, `docs/technical/`, and `docs/editorial/` are not compatibility targets after migration. Skills must use the new `shipglows_data/` paths.
- App docs, changelogs, and package docs may remain outside the umbrella when they are product-facing or ecosystem-required rather than ShipGlows governance docs.
- Versioned symlinks must never contain absolute user homes.
- Central symlinks under `${SHIPGLOWS_DATA_DIR:-$HOME/shipglows_data}` are out of scope for this phase and should not be treated as project documentation truth.
...
- `shipglows_data/technical/metadata-migration-guide.md` must be updated to classify old root docs and new umbrella docs.
- `tools/shipglows_metadata_lint.py` must accept the new canonical locations.
...
Fresh external docs: not needed for this spec. This is internal ShipGlows file architecture and local Git/symlink behavior, based on repo evidence and user architecture direction.

## Invariants

- One project-owned ShipGlows document has one canonical path under project-local `shipglows_data/`.
- Project-local `shipglows_data/` folders are the only source of truth for project documentation in this phase.
- Central `shipglows_data` master files are aggregate/fusion concerns and are ignored for now.
...
- Skills must resolve paths by ownership: ShipGlows source files from `$SHIPGLOWS_ROOT`; project-owned ShipGlows Markdown from the project-local `shipglows_data/`; master aggregate data only when a later phase explicitly reintroduces it.
- Non-owner skills must not normalize doctrine drift by writing to legacy paths. They must route to the owner skill.
- `sg-docs` is the owner for reorganizing existing project documentation into the project-local `shipglows_data/` corpus.
...
- `sg-docs` must gain an explicit reorganize/adopt/update behavior that can move or stage moves for badly organized ShipGlows Markdown.
- `sg-content` must treat editorial docs under `shipglows_data/editorial/` as the canonical editorial governance layer.
- `sg-tasks` must write project-level tracker data under project-local `shipglows_data/` once the project is migrated; master aggregate trackers are deferred.
...
- Existing central `shipglows_data` docs may need a later inventory, but that inventory is not part of this phase.
...
- A project already has a real app-owned `shipglows_data/` directory for another purpose.
- A project already has a root `docs/technical/` used by app docs, not ShipGlows governance.
...
- A project is not a Git repo but still runs under ShipGlows.
- A central `shipglows_data/projects/<project>` directory already contains operational tracker files but is out of scope for this phase.
- Multiple local clones of the same project exist.

## Implementation Tasks

- [ ] Task 1: Decide the canonical folder name and namespace.
  - File: `specs/shipglows-data-project-umbrella-documentation-architecture.md`
  - Action: Record `shipglows_data/` as the canonical project-local folder name and remove alternatives from the spec.
...
- [ ] Task 2: Decide the canonical top-level categories.
  - File: `specs/shipglows-data-project-umbrella-documentation-architecture.md`
  - Action: Record that all project-owned ShipGlows Markdown belongs under `shipglows_data/`, including `technical/`, `editorial/`, `business/`, and `workflow/`.
...
- [ ] Task 2b: Lock root discovery exceptions.
  - File: `skills/references/canonical-paths.md`, `shipglows_data/technical/metadata-migration-guide.md`
...
- [ ] Task 3: Create a path migration matrix.
  - File: `shipglows_data/technical/metadata-migration-guide.md`
  ...
  - Validate with: Matrix covers `BUSINESS.md`, `PRODUCT.md`, `BRANDING.md`, `GTM.md`, `CONTENT_MAP.md`, `docs/technical/`, `docs/editorial/`, `specs/`, `shipglows_data/workflow/bugs/`, `TASKS.md`, `AUDIT_LOG.md`, `PROJECTS.md`, and root discovery exceptions.
...
- [ ] Task 4: Update canonical path doctrine.
  ...
  - Validate with: `rg -n "project umbrella|shipglows_data|canonical" skills/references/canonical-paths.md`.
  - Notes: Keep `$SHIPGLOWS_ROOT` for ShipGlows-owned source files distinct from project umbrella docs.
...
- [ ] Task 6: Add owner-skill routing for non-compliant documentation.
  ...
  - Validate with: `rg -n "non-compliant|sg-docs|reorganize|shipglows_data/" skills`.
...
- [ ] Task 7: Add `sg-docs` reorganization behavior.
  - File: `skills/sg-docs/SKILL.md`, `shipglows_data/technical/metadata-migration-guide.md`
  - Action: Define how `sg-docs` inventories scattered Markdown, classifies ownership, moves or stages moves into project-local `shipglows_data/`, and reports conflicts.
...
- [ ] Task 8: Reconcile tracker doctrine.
  ...
  - Validate with: `rg -n "~/TASKS|local TASKS|symlink to shipglows_data" skills GUIDELINES.md README.md`.
  - Notes: Project-level trackers move under project-local `shipglows_data/workflow/`; master aggregate trackers are deferred.

- [ ] Task 9: Add a symlink and hardcoded-home audit.
  - File: `tools/` and a new or existing test file.
  - Action: Add a check that reports versioned symlinks to `/home/*/shipglows_data`, absolute symlink targets inside Git, and generated templates containing `/home/ubuntu` or `/home/claude`.
...
- [ ] Task 10: Plan project-by-project migration.
  - File: `specs/shipglows-data-project-umbrella-documentation-architecture.md` or a generated migration report.
  ...
  - Validate with: Migration report lists project, current ShipGlows Markdown files, target path, action, conflict status, and validation command.
...
- [ ] New project docs have one canonical umbrella path: `<project>/shipglows_data/`.
- [ ] The central `shipglows_data` repo is not used as a project documentation source of truth in this phase.
- [ ] All relevant skills resolve project ShipGlows Markdown through the new path doctrine.
...
- [ ] `sg-docs` can reorganize badly organized project ShipGlows Markdown into the project-local `shipglows_data/` layout or produce a conflict report.
- [ ] Existing versioned symlinks from projects to `shipglows_data` are removed, replaced by real project docs, or explicitly exempted with reason.
- [ ] No migrated project has duplicate canonical copies of the same ShipGlows decision artifact.
- [ ] Root `AGENT.md`, `CLAUDE.md`, and `README.md` remain real root files; `AGENTS.md` is a symlink to `AGENT.md`.
- [ ] `tools/shipglows_metadata_lint.py` can validate the new paths.
- [ ] A workspace audit can report no versioned symlink to `/home/*/shipglows_data`.
...
- Run `rg -n '/home/(ubuntu|claude)/shipglows_data|~/TASKS.md|local TASKS'` against ShipGlows skills and docs after doctrine updates.
- Run `rg -n "sg-docs|sg-init|non-compliant|reorganize|shipglows_data/" skills` to confirm owner-skill routing is documented.
- Run a pilot migration dry-run before any `git mv`.
- For each migrated project, inspect `git status --short`, `git diff --stat`, and sampled reads from the project-local `shipglows_data/` folder.
...
- Treat the user's phrase "Wispr Flow data" as likely voice transcription for "ShipGlows data" until confirmed.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-09 21:04:33 UTC | sg-spec | GPT-5 Codex | Created draft spec for project-local ShipGlows data umbrella and central symlink architecture. | draft | Answer open questions, then run `/sg-ready specs/shipglows-data-project-umbrella-documentation-architecture.md` |
| 2026-05-09 21:17:17 UTC | sg-spec | GPT-5 Codex | Recorded user doctrine: project-local `shipglows_data/` is the source of truth; all project-owned ShipGlows Markdown belongs there; central master repo and sync are deferred. | draft updated | Decide root compatibility and pilot, then run `/sg-ready specs/shipglows-data-project-umbrella-documentation-architecture.md` |
...
| 2026-05-10 03:01:15 UTC | sg-spec | GPT-5 Codex | Locked root discovery files, workflow folders, project trackers, forced doctrine, git-mv migration default, and dotfiles pilot. | draft decisions locked | Run `/sg-ready specs/shipglows-data-project-umbrella-documentation-architecture.md` |
| 2026-05-10 03:53:00 UTC | sg-ready | GPT-5 Codex | Evaluated readiness gate for project-local `shipglows_data/` umbrella architecture. | not ready | Resolve `Open Questions` to `None`, then rerun `/sg-ready specs/shipglows-data-project-umbrella-documentation-architecture.md` |
| 2026-05-10 03:58:00 UTC | sg-tasks | GPT-5 Codex | Moved future project repo -> master synchronization mechanism out of readiness scope and into task tracking. | draft updated | Rerun `/sg-ready specs/shipglows-data-project-umbrella-documentation-architecture.md` |
| 2026-05-10 08:20:19 UTC | sg-ready | GPT-5 Codex | Re-evaluated readiness after closing open questions and parking future sync in task tracking. | ready | Run `/sg-start ShipGlows Data Project Umbrella Documentation Architecture` |

## Current Chantier Flow

sg-ready passed. The project-local `shipglows_data/` umbrella architecture is ready for implementation. The future project repo -> master synchronization mechanism is tracked as deferred work outside this spec. Next lifecycle step is `/sg-start ShipGlows Data Project Umbrella Documentation Architecture`.
