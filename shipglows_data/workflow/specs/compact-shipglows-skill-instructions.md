...
owner: "unknown"
user_story: "As a ShipGlows operator using Codex and Claude Code skills, I want ShipGlows skill instructions to be shorter, layered, and less repetitive, so agents follow the highest-priority behavior without losing critical governance guardrails."
confidence: medium
risk_level: high
security_impact: yes
docs_impact: yes
...
depends_on:
  - artifact: "shipglows_data/workflow/explorations/2026-05-16-skill-instruction-compaction.md"
    artifact_version: "1.0.0"
...
next_step: "/sg-end Compact ShipGlows Skill Instructions"
---

# Spec: Compact ShipGlows Skill Instructions

## Title

Compact ShipGlows Skill Instructions
...
- Trigger: a maintainer runs `/sg-start Compact ShipGlows Skill Instructions` after this spec passes `/sg-ready`.
...
- Proof of success: `python3 tools/skill_budget_audit.py --skills-root skills --format markdown` shows zero hard violations and reduced body-size risk for the pilot set; metadata lint passes for changed artifacts; `tools/shipglows_sync_skills.sh --check --all` reports current-user runtime visibility.
...
- Create a shared instruction-layering policy for ShipGlows skills.
...
- Rewriting every ShipGlows skill in one pass.
...
- Modifying unrelated CLI behavior in `shipglows.sh`, `lib.sh`, `config.sh`, or `local/`.
...
- Shared references must resolve from `${SHIPGLOWS_ROOT:-$HOME/shipglows}`, not from the current project repo.
- Skill-local references must resolve as `$SHIPGLOWS_ROOT/skills/<skill-name>/references/<file>.md`.
...
- Do not add cross-skill boilerplate to `AGENT.md`, `CLAUDE.md`, or `shipglows_data/technical/context.md`.
- Existing dirty work in unrelated files must not be reverted.

## Dependencies

- Local references and docs:
  - `shipglows_data/workflow/explorations/2026-05-16-skill-instruction-compaction.md`
...
- `shipglows_data/technical/skill-runtime-and-lifecycle.md`: must describe layered skill instructions and any new reference.
- `shipglows_data/technical/code-docs-map.md`: update only if validation commands, code path mappings, or docs triggers change.
- `shipglows_data/workflow/playbooks/spec-driven-workflow.md`: update only if the general workflow doctrine changes, not for pilot implementation details.
...
- Required documentation update: `shipglows_data/technical/skill-runtime-and-lifecycle.md` must mention the skill layering contract and body-size compaction policy.
...
- Conditional documentation update: `shipglows_data/technical/code-docs-map.md` only if the map's validation or trigger language changes.
- Conditional documentation update: `shipglows_data/workflow/playbooks/spec-driven-workflow.md` only if lifecycle doctrine changes.
...
- A skill-local `references/` file shadows a project-local path. Canonical path rules must resolve the ShipGlows-owned file from `$SHIPGLOWS_ROOT`.
...
- [x] Task 1: Capture a fresh baseline and confirm the pilot set
  - File: `shipglows_data/workflow/specs/compact-shipglows-skill-instructions.md`
...
- [x] Task 2: Add the shared skill instruction layering reference
  ...
  - Depends on: Task 1
  - Validate with: `python3 tools/shipglows_metadata_lint.py skills/references/skill-instruction-layering.md`
...
- [x] Task 7: Update technical documentation for the new instruction architecture
  - File: `shipglows_data/technical/skill-runtime-and-lifecycle.md`
  ...
  - Validate with: `rg -n "layer|body-size|skill-instruction-layering|500" shipglows_data/technical/skill-runtime-and-lifecycle.md`
...
- [x] Task 8: Run full validation and runtime visibility checks
  ...
  - Validate with:
    - `python3 tools/skill_budget_audit.py --skills-root skills --format markdown`
    - `python3 tools/shipglows_metadata_lint.py shipglows_data/workflow/specs/compact-shipglows-skill-instructions.md skills/references/skill-instruction-layering.md skills/references/skill-context-budget.md shipglows_data/technical/skill-runtime-and-lifecycle.md`
    - `tools/shipglows_sync_skills.sh --check --all`
...
- [x] CA 4: Given a missing or broken shared reference, when a compacted skill tries to load it, then the final report identifies a ShipGlows installation or reference gap instead of pretending the behavior is verified.
...
- [x] CA 7: Given material skill changes, when `tools/shipglows_sync_skills.sh --check --all` runs, then current-user Claude/Codex skill links are reported as current or the run is blocked with the stale target path.
...
- Metadata lint: `python3 tools/shipglows_metadata_lint.py` on this spec, new references, updated references, and updated technical docs.
- Runtime visibility: `tools/shipglows_sync_skills.sh --check --all`.
- Focused grep checks:
...
- Documentation Update Plan: required for `skills/**`, `skills/references/**`, and `shipglows_data/technical/**` changes.
...
- Stop conditions:
  ...
  - a shared reference cannot be resolved from `$SHIPGLOWS_ROOT`
...
- Recommendation: open follow-up compaction chantier via `/sg-spec Compact ShipGlows Skill Instructions Phase 2` when priority allows.

## sg-verify Verification Notes (2026-05-16)

- Verification verdict: `verified`.
- Main-thread verification corrected two pilot skill references from legacy `specs/*.md` wording to canonical `shipglows_data/workflow/specs/*.md`.
- Checks rerun after correction:
  ...
  - `python3 tools/shipglows_metadata_lint.py shipglows_data/workflow/specs/compact-shipglows-skill-instructions.md skills/references/skill-instruction-layering.md skills/references/skill-context-budget.md shipglows_data/technical/skill-runtime-and-lifecycle.md skills/sg-docs/references/core-governance.md skills/sg-docs/references/mode-playbooks.md skills/sg-audit-design/references/audit-gates.md skills/sg-audit-design/references/audit-checklists.md skills/sg-verify/references/verification-gates.md` passed.
  - `tools/shipglows_sync_skills.sh --check --all` passed with `checked=122 ok=122 blocked=0`.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-16 08:06:03 UTC | sg-spec | GPT-5 Codex | Created spec from `sg-explore` report and local skill inventory audit | draft saved | `/sg-ready Compact ShipGlows Skill Instructions` |
| 2026-05-16 11:46:32 UTC | sg-ready | GPT-5 Codex | Evaluated structure, metadata, user-story fit, adverse/security risk, docs coherence, and execution clarity | ready | `/sg-start Compact ShipGlows Skill Instructions` |
| 2026-05-16 11:55:50 UTC | sg-start | GPT-5 Codex | Implemented layered compaction for pilot skills, added shared/local references, updated technical docs, and ran required validations | implemented | `/sg-verify Compact ShipGlows Skill Instructions` |
| 2026-05-16 12:00:23 UTC | sg-verify | GPT-5 Codex | Verified implementation against user story, success/error behavior, metadata, docs coherence, language doctrine, runtime visibility, and body-size proof | verified | `/sg-end Compact ShipGlows Skill Instructions` |

## Current Chantier Flow

- sg-spec: done, draft saved at `shipglows_data/workflow/specs/compact-shipglows-skill-instructions.md`
- sg-ready: ready
- sg-start: implemented
- sg-verify: verified
- sg-end: not launched
- sg-ship: not launched
- Next command: `/sg-end Compact ShipGlows Skill Instructions`
