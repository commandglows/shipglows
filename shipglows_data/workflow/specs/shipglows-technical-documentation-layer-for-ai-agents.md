...
ShipGlows Technical Documentation Layer for AI Agents

## Status

Ready.

This is a new chantier, distinct from `specs/sg-build-autonomous-master-skill.md`, but compatible with it. The chantier creates the durable technical documentation layer that Technical Readers, Executors, Integrators, and ShipGlows skills use when code changes.

## User Story

En tant qu'utilisatrice ShipGlows qui délègue l'exécution à des agents, je veux que ShipGlows génère et maintienne une documentation technique proche du code, afin que les agents sachent rapidement quoi lire, quoi modifier, quoi valider et quelles docs mettre à jour sans redécouvrir le repo à chaque chantier.
...
ShipGlows must provide a code-proximate technical documentation layer: `docs/technical/` contains an agent-readable internal-only index, `docs/technical/code-docs-map.md` maps code areas to their technical docs and maintenance triggers, each major subsystem has either a dedicated technical doc or an explicit non-coverage reason, and `sg-docs` can generate or audit the layer. After every code-changing execution wave and again during end verification, the Technical Reader produces a `Documentation Update Plan` from the map, but the Technical Reader does not become the primary docs executor; an executor or integrator applies updates sequentially by default, with parallel work allowed only when a spec defines disjoint file ownership. The easy-to-miss edge case is shared documentation such as `code-docs-map.md`: even if subsystem docs can be parallelized, shared maps and final integration must stay sequential, and code cannot ship while mapped technical docs are stale or missing.
...
- If an agent attempts parallel edits on shared files such as `docs/technical/code-docs-map.md`, `AGENT.md`, `CONTEXT.md`, `GUIDELINES.md`, or `shipglows_data/workflow/playbooks/spec-driven-workflow.md`, the work is blocked or rerouted to sequential integration.
...
ShipGlows already has strong orientation and workflow documentation:
...
- `shipglows_data/workflow/playbooks/spec-driven-workflow.md` describes the spec-first doctrine.
- `shipglows_data/technical/metadata-migration-guide.md` documents artifact metadata and frontmatter.
...
Then connect the layer to the existing workflow by updating `skills/sg-docs/SKILL.md`, `AGENT.md`, `CONTEXT.md`, `README.md`, `shipglows_data/workflow/playbooks/spec-driven-workflow.md`, `GUIDELINES.md`, and `tools/shipglows_metadata_lint.py`.
...
`templates/technical_module_context.md` becomes an official linted ShipGlows artifact type. `tools/shipglows_metadata_lint.py` must recognize it and enforce the required frontmatter fields used by governance artifacts, including at least `artifact`, `metadata_schema_version`, `artifact_version`, `status`, `risk_level`, and `docs_impact`.
...
- Create the official taxonomy for ShipGlows code-proximate technical docs.
...
- ShipGlows-owned paths must follow `skills/references/canonical-paths.md`.
...
- `shipglows_data/workflow/playbooks/spec-driven-workflow.md`: spec-first workflow.
...
- [x] Task 3: Create the technical module template.
  ...
  - Depends on : Task 1 plus review of `templates/*.md` and `shipglows_data/technical/metadata-migration-guide.md`.
  ...
  - Notes : This is an official linted artifact type and must include required ShipGlows frontmatter fields.
...
- [x] Task 4: Create the skill-facing technical docs corpus reference.
  ...
  - User story link : Lets ShipGlows skills use technical docs without polluting context.
...
- [x] Task 5: Document runtime CLI.
  ...
  - Action : Document `shipglows.sh`, `lib.sh`, `config.sh`, entrypoints, flows, invariants, side effects, and validations.
  ...
  - Validate with : `rg -n "shipglows.sh|lib.sh|config.sh|Entrypoints|Invariants|Validation|Maintenance Rule" docs/technical/runtime-cli.md`
...
- [x] Task 7: Document skill runtime and lifecycle.
  ...
  - User story link : Allows agents to modify skills without breaking ShipGlows doctrine.
  - Depends on : Tasks 3 and 4 plus reading `skills/*/SKILL.md`, `skills/references/`, and `shipglows_data/workflow/playbooks/spec-driven-workflow.md`.
...
- [x] Task 8: Document artifact metadata and linter.
  ...
  - Action : Document artifact frontmatter, templates, metadata schema, and `tools/shipglows_metadata_lint.py`.
  - User story link : Lets agents create or modify artifacts without breaking metadata validation.
  - Depends on : Task 3 and reading `shipglows_data/technical/metadata-migration-guide.md`, `templates/`, and `tools/shipglows_metadata_lint.py`.
  - Validate with : `rg -n "metadata|frontmatter|artifact|template|shipglows_metadata_lint|Validation|Maintenance Rule" docs/technical/artifact-metadata-and-linter.md`
...
- [x] Task 13: Add technical-docs contract to `sg-docs`.
  ...
  - User story link : Gives ShipGlows an explicit capability for code-proximate documentation maintenance.
...
- [x] Task 17: Update the spec-driven workflow.
  - Fichier : `shipglows_data/workflow/playbooks/spec-driven-workflow.md`
  ...
  - Validate with : `rg -n "Documentation Update Gate|Documentation Update Plan|docs/technical|Technical Reader|executor|integrator" shipglows_data/workflow/playbooks/spec-driven-workflow.md`
...
- [x] Task 19: Review metadata linter compatibility.
  - Fichier : `tools/shipglows_metadata_lint.py`
  - Action : Add support for `technical_module_context` as a linted artifact type and enforce required frontmatter fields.
  - User story link : Keeps new artifacts compatible with ShipGlows checks.
  - Depends on : Tasks 3 and 8.
  - Validate with : Existing metadata-lint command if documented; otherwise `python tools/shipglows_metadata_lint.py --help` and the adapted command.
...
- [x] Task 20: Finalize cross-links and integration.
  - Fichier : `docs/technical/code-docs-map.md`, `AGENT.md`, `CONTEXT.md`, `README.md`, `shipglows_data/workflow/playbooks/spec-driven-workflow.md`, `GUIDELINES.md`
  ...
  - Validate with : `rg -n "TODO|TBD|PLACEHOLDER" docs/technical templates skills/references AGENT.md CONTEXT.md README.md shipglows_data/workflow/playbooks/spec-driven-workflow.md GUIDELINES.md`
...
- `technical_module_context` is recognized by `tools/shipglows_metadata_lint.py` as an official linted artifact type with required governance frontmatter.
...
- `rg -n "technical docs|docs/technical|code-docs-map|Documentation Update Plan" skills/sg-docs/SKILL.md shipglows_data/workflow/playbooks/spec-driven-workflow.md`
...
- Given a fictive diff on `skills/sg-docs/SKILL.md`, the Technical Reader should identify `docs/technical/skill-runtime-and-lifecycle.md`, `skills/references/technical-docs-corpus.md`, and possibly `shipglows_data/workflow/playbooks/spec-driven-workflow.md`.
- Given a fictive diff on `local/`, the Technical Reader should identify `docs/technical/local-tunnels-and-mcp-login.md`.
- Given a fictive diff on `tools/shipglows_metadata_lint.py`, the Technical Reader should identify `docs/technical/artifact-metadata-and-linter.md`.
...
- Wave 3, sequential integration: `skills/sg-docs/SKILL.md`, `AGENT.md`, `CONTEXT.md`, `README.md`, `shipglows_data/workflow/playbooks/spec-driven-workflow.md`, `GUIDELINES.md`, and `tools/shipglows_metadata_lint.py`.
...
- `AGENT.md`, `CONTEXT.md`, `ARCHITECTURE.md`, `GUIDELINES.md`, and `shipglows_data/workflow/playbooks/spec-driven-workflow.md`.
- `shipglows_data/technical/metadata-migration-guide.md`, `templates/*.md`, and `tools/shipglows_metadata_lint.py` before creating `technical_module_context`.
- The owned code area before writing each subsystem doc: `shipglows.sh`, `lib.sh`, `config.sh`, `install.sh`, `local/`, `skills/`, `tools/codebase-mcp/`, or `site/`.
...
- Keep shared files sequential: `docs/technical/code-docs-map.md`, `AGENT.md`, `CONTEXT.md`, `GUIDELINES.md`, `shipglows_data/workflow/playbooks/spec-driven-workflow.md`, and `tools/shipglows_metadata_lint.py`.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-01 09:32:35 UTC | sg-spec | GPT-5 Codex | Created a new chantier spec from user request, delegated exploration, delegated spec drafting, and normalized the final spec. | Draft spec created at `specs/shipglows-technical-documentation-layer-for-ai-agents.md`. | `/sg-ready ShipGlows Technical Documentation Layer for AI Agents` |
| 2026-05-01 09:52:55 UTC | sg-ready | GPT-5 Codex | Evaluated the readiness gate for structure, metadata, user-story alignment, freshness, adversarial gaps, security, execution notes, and open questions. | Not ready: open questions still change scope, workflow, and security decisions; `skills/references/docs-freshness.md` is a missing dependency reference. | `/sg-spec ShipGlows Technical Documentation Layer for AI Agents` |
| 2026-05-01 13:44:46 UTC | sg-spec | GPT-5 Codex | Incorporated user decisions that resolve readiness gaps for agent entrypoint ownership, AGENTS.md compatibility, linted technical module artifacts, internal-only technical docs, Technical Reader cadence, stale-doc shipping policy, freshness tracking, and the freshness reference path. | Spec updated with `Open Questions` set to `None`. | `/sg-ready ShipGlows Technical Documentation Layer for AI Agents` |
| 2026-05-01 13:48:39 UTC | sg-ready | GPT-5 Codex | Re-evaluated the readiness gate after user decisions were incorporated and local metadata checks passed. | Ready: structure, metadata, user-story alignment, task ordering, execution notes, documentation coherence, freshness evidence, adversarial review, security posture, artifact version, and open questions satisfy the gate. | `/sg-start ShipGlows Technical Documentation Layer for AI Agents` |
| 2026-05-01 14:26:00 UTC | sg-start | GPT-5 Codex | Implemented the technical documentation layer, technical module template, technical-docs corpus reference, sg-docs technical mode, metadata-linter support, agent/context/workflow/doc pointers, and AGENTS.md symlink compatibility. | implemented | `/sg-verify ShipGlows Technical Documentation Layer for AI Agents` |
