...
owner: "Diane"
user_story: "As the ShipGlows operator, I want the skill taxonomy analyzed and clarified before shortening descriptions, so skill routing becomes cleaner, less noisy, less duplicated, and easier to reason about."
confidence: medium
risk_level: high
security_impact: yes
docs_impact: yes
...
evidence:
  ...
  - "2026-05-17 sg-start created shipglows_data/workflow/audits/skill-taxonomy-description-audit-2026-05-17.md, edited 56 discovery descriptions, and reduced absolute discovery estimate to 6805/8000 with zero warnings."
...
- Proof of success: `tools/skill_budget_audit.py` shows a measurable discovery-budget reduction; final descriptions remain short one-sentence routing triggers; critical roles remain routable; `tools/shipglows_sync_skills.sh --check --all` passes; changed docs and runtime links remain coherent.
...
- Classify skill families:
  ...
  - pilotage/helper: `shipglows`, `continue`, `sg-context`, `sg-model`, `sg-help`, `sg-status`, `sg-resume`, `sg-tasks`, `sg-backlog`, `sg-priorities`, `sg-review`, `name`
...
- `shipglows_data/technical/skill-runtime-and-lifecycle.md`: technical context for skill families, entrypoints, and runtime visibility.
- Fresh external docs verdict: `fresh-docs not needed` for initial analysis because this chantier edits local ShipGlows artifacts. If implementation changes policy based on current Codex, Claude Code, Claude.ai, or Agent Skills external constraints beyond local references, check the current official docs before modifying policy.
...
- `shipglows` depends on clear descriptions and role boundaries for routing.
...
- `sg-docs`, `sg-skill-build`, `shipglows`, and master lifecycle skills are likely consumers of any taxonomy clarification.
...
- Update `shipglows_data/technical/skill-runtime-and-lifecycle.md` if taxonomy, families, routing rules, or role boundaries change.
...
- [x] Run `tools/shipglows_sync_skills.sh --check --all`.
...
- [x] `shipglows_sync_skills.sh --check --all` passes.
...
- Run `python3 tools/shipglows_metadata_lint.py` on any changed specs or docs with ShipGlows frontmatter.
- Run `tools/shipglows_sync_skills.sh --check --all` after edits.
...
- Keep shared files sequential: `skills/references/chantier-tracking.md`, `skills/references/skill-context-budget.md`, `shipglows_data/technical/skill-runtime-and-lifecycle.md`, and `CHANGELOG.md`.
- Before editing descriptions, write or stage the taxonomy findings in a durable artifact, either this spec, a linked report under `shipglows_data/workflow/audits/`, or a clearly named local working artifact that is committed with the chantier.
...
|---|---|---|---|---|---|
| 2026-05-16 14:38:00 UTC | sg-spec | GPT-5 Codex | Created taxonomy-first skill description compaction spec | draft | /sg-ready shipglows_data/workflow/specs/audit-and-compact-skill-taxonomy-descriptions.md |
| 2026-05-16 14:46:00 UTC | sg-spec | GPT-5 Codex | Integrated local transcript evidence about docs skill failure modes into the spec | draft | /sg-ready shipglows_data/workflow/specs/audit-and-compact-skill-taxonomy-descriptions.md |
...
| 2026-05-16 15:02:00 UTC | sg-spec | GPT-5 Codex | Rewrote the spec in English, added Test Strategy, Execution Notes, and Open Questions, and tightened low-risk change boundaries | draft | /sg-ready shipglows_data/workflow/specs/audit-and-compact-skill-taxonomy-descriptions.md |
