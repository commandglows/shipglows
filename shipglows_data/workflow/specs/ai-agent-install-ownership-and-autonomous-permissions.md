...
evidence:
  ...
  - "L'utilisateur a tranche que ShipGlows doit posseder Claude/Codex/MCP/skills et que le compte non-root de travail doit recevoir une configuration IA autonome sans prompts inutiles."
...
- No-dotfiles fallback: si dotfiles n'a pas prepare `~/.local/share/pnpm` et son PATH, ShipGlows doit preparer le strict minimum par utilisateur cible pour installer ou reutiliser `claude` et `codex` sans creer de fichiers root-owned dans un home non-root; si ce minimum ne peut pas etre etabli proprement, ShipGlows doit stopper pour ce compte avant toute mutation IA.
- Proof of success: `command -v claude`, `command -v codex`, validation JSON/TOML des fichiers generes, inspection des valeurs de permission, verification que `~/.local/share/pnpm` ou un chemin equivalent choisi par ShipGlows est resolu pour l'utilisateur cible quand un install user-local est necessaire, rapport d'installation ShipGlows indiquant `claude` et `codex` comme geres par ShipGlows, et absence des blocs MCP/Codex/Claude geres par dotfiles apres reexecution de dotfiles.

## Error Behavior

- If no non-root operational user exists, ShipGlows completes only the system/global setup, prints a clear recommendation to create a non-root operational user, and does not silently apply autonomous AI config to root.
- If root is selected for autonomous AI config, the installer requires an explicit interactive confirmation or non-interactive opt-in such as `SHIPGLOWS_AI_ALLOW_ROOT_AUTONOMOUS=1`; otherwise root can receive standard ShipGlows config without autonomous permissions.
- If `SHIPGLOWS_INSTALL_USERS_MODE=user-list` is provided with an empty list, duplicate usernames, unknown usernames, service accounts, locked accounts, no-login accounts, or accounts whose home cannot be resolved to the selected account's real personal directory, the installer rejects those entries explicitly, marks them `invalid` or `rejected`, and performs no config mutation for them.
...
- If dotfiles is absent and ShipGlows cannot establish a safe user-local npm prefix and PATH for the target account, the installer reports that account as blocked before any Claude/Codex install attempt and does not write partial AI config.
...
The current installation boundary is inconsistent. ShipGlows already configures Claude/Codex settings, MCP servers, skills, statusline, aliases and `shipglows_data` for root and every `/home/*` account, but its report still says `claude` and `codex` are managed by dotfiles. Dotfiles still installs Codex in `npm-tools`, installs Claude/Codex again in `install_ai_tools`, configures Claude/Codex MCP, symlinks a Codex config template, and links Claude skills. This creates double ownership, stale or conflicting config blocks, and a poor daily workflow where the non-root operator can still receive permission prompts from Codex or Claude.

## Solution

Make ShipGlows the sole owner of Claude Code, OpenAI Codex, ShipGlows MCP registration, ShipGlows skills and autonomous agent permissions. Move Claude/Codex binary installation or validation into ShipGlows user-scope setup, add explicit user targeting before per-user configuration, apply autonomous permissions only to selected operational profiles with root guarded by explicit opt-in, and remove Claude/Codex/MCP/skills responsibilities from dotfiles while keeping dotfiles focused on generic shell/editor/tooling.

## Scope In

- ShipGlows `install.sh` owns installation or verification of `@anthropic-ai/claude-code` and `@openai/codex`.
...
- Rewriting all ShipGlows install architecture.
- Rewriting all dotfiles component taxonomy beyond Claude/Codex/MCP/ShipGlows-owned AI artifacts.
- Managing all third-party AI tools such as Kilocode, OpenCode or Gemini unless they are explicitly used by ShipGlows workflow.
...
- ShipGlows remains root-capable for system/global setup but day-to-day autonomous AI configuration is intended for a regular non-root operational user.
- User-scope writes must run with correct ownership or be followed by safe `chown` of only ShipGlows-created paths.
- Existing user custom settings outside ShipGlows-managed blocks must be preserved.
...
- ShipGlows must be able to bootstrap the minimal user-local npm prefix/PATH needed for Claude/Codex ownership even when dotfiles was never run, or else fail closed for that user before any AI mutation.
...
- One ownership source for Claude/Codex/MCP/ShipGlows skills: ShipGlows.
- dotfiles must not mutate `~/.claude` or `~/.codex` for ShipGlows-owned behavior after migration.
...
- Project dependencies are never installed globally by ShipGlows or dotfiles.
- Re-running ShipGlows and dotfiles is safe and does not reintroduce duplicate config blocks.

## Links & Consequences

- Upstream:
  - `shipglows/install.sh` system setup provides Node/npm and existing MCP configuration helpers.
  - dotfiles may create/prep the non-root user and generic PATH/npm prefix before ShipGlows runs.
- Downstream:
  - `~/.claude/settings.json`, `~/.codex/config.toml`, `~/.bashrc`, `~/.claude/skills`, `~/.codex/skills`, `~/shipglows_data`.
  - ShipGlows README and dotfiles README install guidance.
  - Existing install reports and `shipglows_data/workflow/audits/2026-04-28-install-run-trace.md` need consistent terminology.
- Operational consequences:
  - When dotfiles was never run, ShipGlows may need to create or repair the per-user PATH/bootstrap lines required for `~/.local/share/pnpm` in the selected account's `~/.bashrc` before attempting user-local AI installs.
  - Existing users with dotfiles-managed Codex symlinks must be migrated to real user config or have the symlink removed before ShipGlows writes.
...
- Update `shipglows/README.md`:
  - ShipGlows owns Claude/Codex install/config, MCP, skills and autonomous permissions.
...
- Server with multiple real users: interactive mode lists candidates and configures only selected users; non-interactive mode requires `SHIPGLOWS_INSTALL_USERS_MODE=user-list` and `SHIPGLOWS_INSTALL_USERS`.
- `SHIPGLOWS_INSTALL_USERS` contains unknown, duplicate, or ineligible usernames: ShipGlows rejects them deterministically, reports the rejected entries, and does not silently fall back to all-users or root.
- A local account exists but uses `/usr/sbin/nologin`, `/bin/false`, is locked, or has a missing/unwritable personal directory: ShipGlows treats it as ineligible for autonomous config.
- Existing `~/.codex/config.toml` is a symlink to dotfiles: ShipGlows must replace or migrate it safely before writing, preserving user custom content where possible.
- Existing dotfiles MCP block exists in Codex config: ShipGlows removes only the `# >>> dotfiles codex mcp >>>` managed block or matching dotfiles-managed sections.
- Existing Claude MCP entries created by dotfiles CLI: ShipGlows must decide by ownership markers or known server names and avoid destructive removal of unrelated user MCPs.
- User has custom Claude permission deny rules: ShipGlows must preserve them unless they conflict with autonomous defaults, then report the conflict instead of deleting.
- npm global prefix points to root because ShipGlows runs with sudo: user-scope install must run as the target user or explicitly set target user's npm prefix.
- dotfiles was never run for the target account: ShipGlows must create the minimal local npm prefix/PATH bootstrap it needs in that account's `~/.bashrc`, or fail closed before attempting user-local installs.
- `claude` or `codex` already installed globally: ShipGlows can reuse if visible to the target user, but report source and avoid duplicate user-local installs unless needed.
...
- [ ] Task 1: Consolidate the ownership spec references
  - File: `shipglows_data/workflow/specs/ai-agent-install-ownership-and-autonomous-permissions.md`
  ...
  - Depends on: None
  - Validate with: `rg -n "superseded|ai-agent-install-ownership" shipglows_data/workflow/specs/ai-agent-install-ownership-and-autonomous-permissions.md shipglows_data/workflow/archives/repository-history/root-documentation/INSTALLATION-OWNERSHIP-SPEC.md`
...
- [ ] Task 3: Add explicit user target selection in ShipGlows
  - File: `install.sh`
  - Action: replace the unconditional `setup_user "$HOME" "root"` plus `/home/*` loop with a target selection phase supporting interactive selection and env-driven non-interactive modes: `SHIPGLOWS_INSTALL_USERS_MODE=all-users|user-list` and `SHIPGLOWS_INSTALL_USERS`.
...
- [ ] Task 4: Add operational user detection and root guard
  ...
  - Notes: Do not create users in ShipGlows by default; defer to dotfiles/bootstrap guidance unless a future spec changes that.

- [ ] Task 5: Bootstrap minimal user-local PATH and npm prefix when dotfiles is absent
  - File: `install.sh`
  - Action: add a narrow fallback that ensures the selected user's local npm prefix and PATH are usable for ShipGlows-owned AI tools when dotfiles never prepared them; write only the minimum PATH/bootstrap lines ShipGlows needs into that user's `~/.bashrc` and never claim generic tooling ownership.
  - User story link: allows ShipGlows to configure the intended non-root operator even when dotfiles is absent.
  - Depends on: Task 4
  - Validate with: `sudo -u <user> bash -lc 'npm config get prefix && command -v npm'`; inspect `~/.bashrc` for one ShipGlows-managed PATH bootstrap block only when needed.
...
- [ ] Task 6: Install or verify Claude/Codex per selected user
  ...
  - User story link: ShipGlows becomes the actual owner of daily AI coding tools.
...
- [ ] Task 9: Add autonomous and safe aliases
  ...
  - Notes: Prefer a delimited ShipGlows block over unstructured appends.

- [ ] Task 10: Update ShipGlows report and verification output
  - File: `install.sh`
  - Action: change report rows for `claude` and `codex` from `NON_APPLICABLE | gere par dotfiles` to ShipGlows-managed per-user statuses, including autonomous mode, eligible users, rejected users, skipped users, and blocked users when PATH/npm fallback could not be established.
...
- [ ] Task 11: Remove Claude/Codex ownership from dotfiles install flow
  ...
  - Action: remove `@openai/codex` from `install_npm_tools`, remove or no-op `install_ai_tools` for Claude/Codex, remove Claude/Codex MCP mutation, remove Codex config symlink and Claude skills symlink, and remove ShipGlows-owned AI aliases.
...
- [ ] Task 12: Update dotfiles helper/component metadata
  ...
  - Action: remove or reclassify dotfiles component entries for `claude-code`, Claude/Codex MCP, ShipGlows-owned aliases and health checks so menus/docs do not offer actions dotfiles no longer owns.
...
- [ ] Task 13: Update ShipGlows documentation
  - File: `README.md`
  - Action: document ShipGlows ownership of Claude/Codex/MCP/skills, selected-user install behavior, eligibility and rejection rules, no-dotfiles fallback behavior, autonomous defaults, root guard and safe aliases.
  - User story link: operators understand what ShipGlows changes and where daily IA config lives.
  - Depends on: Task 3 through Task 10
  - Validate with: `rg -n 'Claude|Codex|autonomous|bypassPermissions|approval_policy|SHIPGLOWS_INSTALL_USERS' README.md`
  - Notes: Replace stale `root + /home/*` wording.

- [ ] Task 14: Update dotfiles documentation
  - File: `/home/ubuntu/dotfiles/README.md`
  - Action: state that dotfiles owns generic tooling only and delegates Claude/Codex/MCP/ShipGlows skills to ShipGlows.
  - User story link: prevents operators from running the wrong script for AI config.
  - Depends on: Task 11, Task 12
  - Validate with: `rg -n 'Claude Code Skills|Codex|MCP|ShipGlows' /home/ubuntu/dotfiles/README.md`
...
- [ ] CA 1: Given a fresh ShipGlows install with one non-root operational user selected, when installation completes, then `claude` and `codex` are available in that user's PATH and reported as ShipGlows-managed.
- [ ] CA 2: Given a selected non-root user, when ShipGlows configures Claude, then `~/.claude/settings.json` contains `permissions.defaultMode = "bypassPermissions"` and `permissions.skipDangerousModePermissionPrompt = true` while preserving existing MCP/statusLine settings.
- [ ] CA 3: Given a selected non-root user, when ShipGlows configures Codex, then `~/.codex/config.toml` is valid TOML and contains one effective `approval_policy = "never"` and one effective `sandbox_mode = "danger-full-access"`.
- [ ] CA 4: Given root is not explicitly opted into autonomous mode, when ShipGlows runs as root, then root is not silently configured for autonomous AI permissions.
- [ ] CA 5: Given no non-root operational user exists, when ShipGlows runs, then system setup may complete but user-scope autonomous AI config is skipped with a clear recommendation to create/select a non-root user.
- [ ] CA 6: Given multiple `/home/*` users exist, when the operator selects only one, then only that user's `.claude`, `.codex`, `.bashrc` and `shipglows_data` are changed.
- [ ] CA 7: Given non-interactive mode uses `SHIPGLOWS_INSTALL_USERS_MODE=user-list`, when `SHIPGLOWS_INSTALL_USERS` contains valid users, then only those users are configured and the report lists skipped users.
- [ ] CA 8: Given non-interactive mode uses `SHIPGLOWS_INSTALL_USERS_MODE=user-list`, when `SHIPGLOWS_INSTALL_USERS` contains an unknown, duplicate, service, no-login, locked, `root`, or unresolved-home account, then that entry is rejected explicitly and receives no config mutation.
- [ ] CA 9: Given dotfiles was never run for a selected user, when ShipGlows needs a user-local Claude/Codex install, then ShipGlows creates only the minimum PATH/npm-prefix bootstrap needed for that user or marks that user blocked before any AI config write.
- [ ] CA 9b: Given ShipGlows configures one selected account, when the install completes, then no per-user file outside that selected account's personal directory was created or modified as part of that user's AI setup.
- [ ] CA 10: Given `~/.codex/config.toml` is a symlink to dotfiles, when ShipGlows migrates it, then Codex config becomes writable by the target user and dotfiles no longer owns the file.
- [ ] CA 11: Given dotfiles is re-run after migration, when it finishes, then it does not install Claude/Codex, does not mutate `~/.claude` or `~/.codex` for ShipGlows-owned settings, and does not recreate ShipGlows-owned AI aliases.
...
- [ ] CA 14: Given documentation is read after implementation, when an operator compares ShipGlows and dotfiles docs, then only ShipGlows claims Claude/Codex/MCP/skills ownership.
...
- Integration with temporary homes:
  ...
  - invalid usernames and duplicate usernames in `SHIPGLOWS_INSTALL_USERS`.
...
- Compatibility: medium. Existing scripts may rely on `./install.sh --only=mcp` in dotfiles. Mitigation: dotfiles should print the ShipGlows command for Claude/Codex/MCP instead of silently doing nothing.
...
- Approach:
  1. Implement ShipGlows user targeting and root guard before changing permissions.
  ...
  4. Add ShipGlows-owned Claude/Codex binary install/validation.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-04-28 22:27:54 UTC | sg-spec | GPT-5 Codex | Created canonical spec for ShipGlows ownership of Claude/Codex installation, autonomous permissions, user targeting, and dotfiles overlap removal | draft | /sg-ready AI agent install ownership and autonomous permissions |
...
| 2026-04-28 23:28:00 UTC | sg-spec | GPT-5 Codex | Updated the spec to define target-user eligibility, invalid-selection handling, and ShipGlows fallback when dotfiles did not prepare user-local npm/PATH | draft | /sg-ready AI agent install ownership and autonomous permissions |
...
| 2026-04-29 02:00:00 UTC | sg-start | GPT-5 Codex | Implemented ShipGlows ownership changes for user targeting, autonomous Claude/Codex permissions, per-user bootstrap, dotfiles overlap removal, and ownership docs updates | implemented | /sg-verify AI agent install ownership and autonomous permissions |
