...
owner: "unknown"
user_story: "As a ShipGlows operator auditing and running Flutter projects, I want ShipGlows to provision Flutter and Dart runtimes through each project's Flox environment, so audits, launches, and verification commands do not fail just because Flutter is absent from the global PATH."
risk_level: "medium"
confidence: "high"
security_impact: "yes"
docs_impact: "yes"
...
next_step: "/sg-start Flutter Dart Flox Provisioning for ShipGlows Projects"
---

# Spec: Flutter Dart Flox Provisioning for ShipGlows Projects

## Title

Flutter Dart Flox Provisioning for ShipGlows Projects
...
As a ShipGlows operator auditing and running Flutter projects, I want ShipGlows to provision Flutter and Dart runtimes through each project's Flox environment, so audits, launches, and verification commands do not fail just because Flutter is absent from the global PATH.

## Minimal Behavior Contract

When ShipGlows detects a Dart or Flutter project through `pubspec.yaml`, starting or preparing that project must ensure the required runtime is present in the project's Flox environment before any `dart`, `flutter`, audit, or PM2 launch command is treated as valid. If the Flox environment is missing, ShipGlows creates it and installs the configured runtime packages; if the environment already exists but lacks the runtime, ShipGlows repairs it idempotently. On failure, ShipGlows must stop with an actionable diagnostic naming the project path, detected project kind, package string attempted, and the operator command to retry or override. The easy edge case to miss is an existing `.flox` directory that was created earlier but contains no Flutter package: returning early from `init_flox_env` must not continue to leave that project broken.

## Success Behavior

- Preconditions: `flox` is installed, a project has `pubspec.yaml`, and the project is detected as `flutter` or plain `dart`.
- Trigger: an operator starts the project through ShipGlows, or runs the new focused runtime-provisioning path during implementation validation.
- User/operator result: the operator sees that ShipGlows installed or verified the Dart/Flutter runtime for that project; `flox activate -- flutter --version` works for Flutter projects and `flox activate -- dart --version` works for Dart projects.
...
- User/operator response: ShipGlows prints a direct error with the detected kind, target directory, package string, and a retry hint such as `flox install -d <project> <package>` or the relevant `SHIPGLOWS_FLOX_*_PACKAGES` override.
...
- Must never happen: ShipGlows must not tell the operator to install Flutter globally as the primary fix for a project Flox environment that it can repair itself.
...
The operational consequence is that code audits, `flutter analyze`, `flutter test`, and Flutter web launch checks get blocked by local tooling rather than by app defects. With several Flutter projects now active, the correct fix is to make ShipGlows's Flox provisioning path own Flutter and Dart runtimes.
...
- Add `SHIPGLOWS_FLOX_DART_PACKAGES` and `SHIPGLOWS_FLOX_FLUTTER_PACKAGES` defaults in `config.sh`.
...
- Do not make Flutter a required global ShipGlows prerequisite.
...
- Downstream:
  ...
  - Existing projects with blank `.flox` manifests get repaired on next ShipGlows provisioning path.
- Regression surface:
...
- [ ] Task 1: Add configurable Dart and Flutter Flox package defaults.
  - File: `config.sh`
  - Action: Add `SHIPGLOWS_FLOX_DART_PACKAGES` and `SHIPGLOWS_FLOX_FLUTTER_PACKAGES` near existing Flox package defaults. Use `dart` for Dart and `flutter@3.41.5-sdk-links` for Flutter unless readiness changes the version with evidence.
  - User story link: Gives ShipGlows a durable runtime contract instead of relying on global PATH.
  - Depends on: none
  - Validate with: `bash -n config.sh`; `source config.sh && printf '%s\n' "$SHIPGLOWS_FLOX_FLUTTER_PACKAGES"`
...
- [ ] Task 2: Add an idempotent helper to ensure Flox runtime packages for a project.
  ...
  - Depends on: Task 1
  - Validate with: `bash -n lib.sh`; focused shell smoke with a temporary Flox env if practical; negative checks with `SHIPGLOWS_FLOX_FLUTTER_PACKAGES='flutter;touch /tmp/bad'` and `SHIPGLOWS_FLOX_FLUTTER_PACKAGES='--help'` must fail before `flox install`.
...
- [ ] Task 7: Update technical docs mapped to touched code.
  ...
  - Depends on: Task 4
  - Validate with: `python3 tools/shipglows_metadata_lint.py docs/technical/runtime-cli.md`
...
- [ ] Task 8: Update installer scope docs.
  ...
  - Depends on: Task 6
  - Validate with: `python3 tools/shipglows_metadata_lint.py docs/technical/installer-and-user-scope.md`
...
- [ ] Task 9: Run local remediation or validation against current Flutter projects.
  ...
  - Action: After code changes, validate the new behavior against a temp fixture and optionally run the repair path on `/home/ubuntu/contentflow/contentflow_app`, `/home/ubuntu/shipglows_app`, or `/home/ubuntu/tubeflow_flutter/tubeflow_app` if it is safe and explicit in the implementation report.
...
- [ ] CA 1: Given a Flutter project with no `.flox`, when ShipGlows initializes it, then the resulting project env contains the configured Flutter package and `flox activate -- flutter --version` succeeds.
- [ ] CA 2: Given a Flutter project with an existing `.flox` that lacks Flutter, when ShipGlows prepares or starts it, then ShipGlows installs or verifies Flutter in that env instead of returning early.
- [ ] CA 3: Given a plain Dart project, when ShipGlows initializes or repairs the env, then the configured Dart package is installed and `flox activate -- dart --version` succeeds.
- [ ] CA 4: Given global `flutter` is absent but project Flox can provide Flutter, when the operator starts the project, then ShipGlows does not instruct global Flutter installation as the primary fix.
- [ ] CA 5: Given the configured Flutter package is invalid or unavailable, when provisioning runs, then ShipGlows stops before PM2 launch and prints project path, detected kind, package string, and retry or override guidance.
- [ ] CA 6: Given a Flutter project has no `web/` directory or custom PM2/build script, when runtime provisioning succeeds, then ShipGlows still reports the existing no-web-target error instead of masking it as a runtime failure.
...
- [ ] CA 10: Given `SHIPGLOWS_FLOX_FLUTTER_PACKAGES` contains a path, flake installable, option-like token beginning with `-` such as `--help`, quote, shell metacharacter, command substitution, or empty token, when provisioning runs, then ShipGlows rejects the override before invoking `flox install`, prints an actionable French diagnostic, and does not start PM2.
...
- Static:
  ...
  - `python3 tools/shipglows_metadata_lint.py docs/technical/runtime-cli.md docs/technical/installer-and-user-scope.md`
  - `rg -n "SHIPGLOWS_FLOX_DART_PACKAGES|SHIPGLOWS_FLOX_FLUTTER_PACKAGES|ensure_flox_runtime|flutter@|Install SDK|flox install -d" config.sh lib.sh README.md docs/technical`
- Focused shell:
...
- Live validation:
  ...
  - If validating a current project, prefer `contentflow_app` or `shipglows_app` because they already have `.flox` but lack Flutter; report exactly which one was touched.
- Negative validation:
  - Temporarily set `SHIPGLOWS_FLOX_FLUTTER_PACKAGES=definitely-not-a-real-package` in a temp project and verify failure is visible and actionable.
  - Temporarily set `SHIPGLOWS_FLOX_FLUTTER_PACKAGES='flutter;touch /tmp/shipglows-bad'` in a temp project and verify the unsafe token is rejected before `flox install`, without creating the marker file.
  - Temporarily set `SHIPGLOWS_FLOX_FLUTTER_PACKAGES='--help'` and `SHIPGLOWS_FLOX_FLUTTER_PACKAGES='-v'` in a temp project and verify both option-like tokens are rejected before `flox install`.
- Language validation:
...
- Packages to avoid:
  ...
  - Do not add npm, Python, or Dart package dependencies to ShipGlows for this change.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-05-02 11:53:16 UTC | sg-spec | GPT-5 Codex | Created a spec for durable Flutter/Dart runtime provisioning through project Flox environments, based on local audit blockers and current Flox docs. | draft saved | /sg-ready Flutter Dart Flox Provisioning for ShipGlows Projects |
| 2026-05-02 12:33:17 UTC | sg-ready | GPT-5 Codex | Evaluated readiness against the spec-driven Definition of Ready, local Flox evidence, language doctrine, fresh Flox docs, adversarial workflow risks, and proportional security review. | not ready | /sg-spec Flutter Dart Flox Provisioning for ShipGlows Projects |
| 2026-05-02 12:38:44 UTC | sg-spec | GPT-5 Codex | Updated the spec after readiness review and user decision to follow the strict package override recommendation, adding security mitigations, language doctrine, corrected Flox evidence, and closed open questions. | draft updated | /sg-ready Flutter Dart Flox Provisioning for ShipGlows Projects |
| 2026-05-02 12:46:37 UTC | sg-ready | GPT-5 Codex | Re-evaluated readiness after the sg-spec update and found one remaining package-token validation gap: option-like tokens beginning with `-` are not explicitly rejected by the spec's allowlist. | not ready | /sg-spec Flutter Dart Flox Provisioning for ShipGlows Projects |
| 2026-05-02 12:50:06 UTC | sg-spec | GPT-5 Codex | Updated the package-token contract to reject option-like leading-dash tokens before `flox install`, added negative validation cases, and routed the chantier back to readiness. | draft updated | /sg-ready Flutter Dart Flox Provisioning for ShipGlows Projects |
| 2026-05-02 13:12:47 UTC | sg-ready | GPT-5 Codex | Re-evaluated the updated spec against structure, user-story alignment, execution readiness, language doctrine, fresh Flox docs, adversarial workflow risks, and security guardrails. | ready | /sg-start Flutter Dart Flox Provisioning for ShipGlows Projects |
| 2026-05-02 13:33:32 UTC | sg-start | GPT-5 Codex | Implemented Flutter/Dart project Flox runtime provisioning with strict override validation, existing `.flox` repair path, focused runtime tests, and runtime/installer docs updates. | implemented | /sg-verify Flutter Dart Flox Provisioning for ShipGlows Projects |
| 2026-05-02 13:41:27 UTC | sg-verify | GPT-5 Codex | Verified implementation against the ready spec, corrected stable gaps in override preservation and global SDK fallback commands, ran static checks, focused tests, metadata lint, fresh Flox docs check, and live temp `.flox` Flutter repair validation. | verified | /sg-end Flutter Dart Flox Provisioning for ShipGlows Projects |
