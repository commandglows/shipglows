...
updated: "2026-08-28"
updated_at: "2026-09-03 13:58:44 UTC"
status: reviewed
...
evidence:
  ...
  - "Operator decision 2026-08-28: keep Unix s u for system packages; ShipGlows runtime updates use an explicit update target, while the native Windows launcher routes operators to it."
  - "Operator decision 2026-09-03: bare shipglows update is non-mutating guidance; update targets follow the shipglows update <target> grammar."
...
`shipglows update status` is read-only and reports the active source, channel, and required reload. `shipglows update runtime` selects the stable official bootstrap for a normal installation, or the current clean remote-tracking branch for a linked developer checkout. Bare `shipglows update` performs no mutation and lists the explicit targets supported by the current platform; Windows includes runtime, skills, tools, and status, while Unix includes runtime and status. Linked skills are reported as live rather than copied or reinstalled. Native Windows exposes the runtime operation through the explicit command while its native launcher refuses self-update. Unix preserves `s u` as the package-update action.
...
- The Windows launcher accepts `shipglows update runtime` and delegates to the DevServer implementation; bare `shipglows update` lists explicit targets without mutation.
...
- `UPDATE-WINDOWS-LAUNCHER`: `shipglows update runtime` reaches the DevServer update implementation while native self-update routes to that command.
- `UPDATE-UNIX-COMPATIBILITY`: `shipglows update runtime` is reserved for the ShipGlows runtime while `s u` remains the system-package action.
...
- [x] Document the TUI as a read-only route to `shipglows update runtime` and update the public router guidance.
...
| 2026-09-03 | sg-development | GPT-5.6 Codex | Normalized update commands to the `shipglows update <target>` grammar, retaining only platform-supported targets. | verified locally | Deliver through the protected dev pull-request gate. |
