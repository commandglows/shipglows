...
scope: "feature"
owner: "Diane"
user_story: "As a ShipGlows maintainer creating or changing skills, I want one master skill to orchestrate the skill lifecycle from spec through SKILL.md edits, refresh, budget audit, verification, public documentation, and ship, so ShipGlows skills stay product-coherent, technically safe, discoverable, and publicly documented."
confidence: high
risk_level: high
security_impact: yes
docs_impact: yes
...
supersedes: []
evidence:
  - "Historical migration-source provenance (legacy root documents): BRANDING.md, BUSINESS.md, CONTENT_MAP.md, CONTEXT.md, GTM.md, GUIDELINES.md, PRODUCT.md."
  ...
  - "Repo investigation 2026-05-02: skills are maintained under skills/*/SKILL.md with canonical ShipGlows path resolution from skills/references/canonical-paths.md."
...
- Must never happen: create a duplicate skill for an already covered workflow, rename a skill invocation key without explicit approval, hide argument semantics in an overlong description, skip budget audit after adding or materially expanding a skill, publish a public skill page that promises behavior not present in `SKILL.md`, add ShipGlows governance frontmatter to `site/src/content/skills/*.md`, ship stale `sg-help` or stale public skill catalog text, or commit unrelated dirty files.
- Silent failure: not allowed. Every failed gate must produce a visible status and a recovery command.

## Problem

ShipGlows now has enough skills that creating or changing a skill is a cross-system product operation, not a local Markdown edit. A skill affects agent discovery, lifecycle doctrine, public explanations, `sg-help`, budget constraints, technical docs, editorial surfaces, and the public skills catalog.
...
- Require canonical path resolution through `${SHIPGLOWS_ROOT:-$HOME/shipglows}` for ShipGlows-owned skills, references, tools, and workflow docs.
- Create or verify current-user runtime symlinks for new or renamed skills: `~/.claude/skills/<name>` and `~/.codex/skills/<name>` must resolve to `${SHIPGLOWS_ROOT:-$HOME/shipglows}/skills/<name>` and expose `SKILL.md`.
...
- Keep `description` compact, one sentence, trigger-focused, and within the ShipGlows budget thresholds in `skills/references/skill-context-budget.md`.
...
- Run `python3 tools/shipglows_metadata_lint.py` on changed ShipGlows artifacts and at least the chantier spec.
...
- Update `README.md` and `shipglows_data/workflow/playbooks/spec-driven-workflow.md` when the skill changes official workflow doctrine, recommended entrypoints, or lifecycle routing.
...
- Run focused public-site checks when public skill content changes, including `pnpm --dir shipglows-site build` and relevant `rg` checks for schema, internal-only leaks, stale skill names, and public claim drift.
...
- Do not create a generic third-party skill generator outside ShipGlows's skill library.
...
- Do not add ShipGlows artifact metadata to Astro runtime content under `site/src/content/skills/*.md`.
...
- Shared files are sequential integration surfaces: `skills/sg-help/SKILL.md`, `README.md`, `shipglows_data/workflow/playbooks/spec-driven-workflow.md`, `CONTENT_MAP.md`, `docs/editorial/**`, `docs/technical/code-docs-map.md`, `site/src/content.config.ts`, and public hub copy.
...
- Local tools: `tools/skill_budget_audit.py` and `tools/shipglows_metadata_lint.py`.
...
- Public site runtime: Astro skills collection in `shipglows-site/src/content.config.ts`, public skill content in `shipglows-site/src/content/skills/`, hub route `shipglows-site/src/pages/skills/index.astro`, detail route `shipglows-site/src/pages/skills/[slug].astro`, and build command `pnpm --dir shipglows-site build`.
...
- Fresh external docs verdict: fresh-docs checked on 2026-05-02 for Astro runtime assumptions used by this spec. Local version discovered from `shipglows-site/pnpm-lock.yaml`: `astro@6.4.8`. Official docs consulted: content collections guide (`https://docs.astro.build/en/guides/content-collections/`) and build/deploy guidance (`https://docs.astro.build/en/develop-and-build/`, `https://docs.astro.build/en/guides/deploy/`). Decision impact: keep schema-first content constraints in `shipglows-site/src/content.config.ts` and keep `pnpm --dir shipglows-site build` as a required validation gate when public skill content or routes change.
...
- Workflow docs: update `shipglows_data/workflow/playbooks/spec-driven-workflow.md` when `sg-skill-build` becomes an official recommended entrypoint or changes lifecycle doctrine.
...
- Public skill page: create or update `site/src/content/skills/sg-skill-build.md` if the skill is public-facing; follow `site/src/content.config.ts` exactly and omit ShipGlows governance metadata.
...
- A public skill page tries to include `metadata_schema_version` or other ShipGlows artifact metadata; preserve the Astro schema and put governance metadata in the spec or docs instead.
...
- [x] Task 1: Confirm the target master skill contract and existing overlap
  ...
  - Action: Re-read this spec, search existing skills for overlap with `sg-build`, `sg-skills-refresh`, and system `skill-creator`, then confirm that the new ShipGlows skill name remains `sg-skill-build`.
  - User story link: Prevents duplicate or confusing skill entrypoints.
  - Depends on: None
  - Validate with: `rg -n "sg-skill-build|skill-build|skills-refresh|skill-creator|sg-build" skills specs site/src/content/skills README.md shipglows_data/workflow/playbooks/spec-driven-workflow.md`
...
- [x] Task 4: Add public skill content
  ...
  - Depends on: Task 2
  - Validate with: `pnpm --dir shipglows-site build`
  - Notes: Use the Astro skills schema only. Do not add ShipGlows artifact metadata to this runtime content file.
...
- [x] Task 7: Update workflow and repo docs if the entrypoint becomes official
  - File: `README.md`, `shipglows_data/workflow/playbooks/spec-driven-workflow.md`
  ...
  - Validate with: `rg -n "sg-skill-build|skill maintenance|skill creation|skills-refresh|skill budget" README.md shipglows_data/workflow/playbooks/spec-driven-workflow.md`
...
- [x] Task 11: Run budget, metadata, and content validation
  ...
  - Validate with: `python3 tools/skill_budget_audit.py --skills-root skills --format markdown`; `python3 tools/shipglows_metadata_lint.py specs/sg-skill-build-master-skill.md README.md shipglows_data/workflow/playbooks/spec-driven-workflow.md CONTENT_MAP.md docs/technical docs/editorial`; `pnpm --dir shipglows-site build`; `rg -n "docs/technical|secret|token|credential" shipglows-site/src CONTENT_MAP.md`
...
- [ ] AC 1: Given a user asks to create a new ShipGlows skill, when `sg-skill-build` runs, then it starts or attaches to a spec before creating `skills/<name>/SKILL.md`.
...
- [ ] AC 10: Given official workflow docs are read, when `sg-skill-build` changes lifecycle doctrine, then README and `shipglows_data/workflow/playbooks/spec-driven-workflow.md` are updated or explicitly marked no-impact.
...
- [ ] AC 16: Given a new skill is created or a skill invocation directory changes, when validation runs, then current-user `~/.claude/skills/<name>` and `~/.codex/skills/<name>` symlinks resolve to the ShipGlows skill directory and expose `SKILL.md`, or the run blocks with a concrete reason.
...
- Runtime link validation: verify `readlink -f ~/.claude/skills/<name>` and `readlink -f ~/.codex/skills/<name>` match `${SHIPGLOWS_ROOT:-$HOME/shipglows}/skills/<name>`, and verify both expose `SKILL.md`.
- Metadata validation: run `python3 tools/shipglows_metadata_lint.py` on the spec and changed ShipGlows artifacts; include `docs/technical` and `docs/editorial` when they are edited.
- Public site validation: run `pnpm --dir shipglows-site build` when `shipglows-site/src/content/skills/`, `shipglows-site/src/content.config.ts`, or public skill routes change.
...
- Skill overlap risk: `sg-skill-build` can confuse users if it overlaps too much with `sg-build`, `sg-skills-refresh`, or system `skill-creator`. Mitigation: define it specifically as ShipGlows skill lifecycle orchestration.
