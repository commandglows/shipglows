...
scope: feature
owner: Diane
user_story: "En tant qu'utilisatrice ShipGlows qui corrige des bugs avec des agents sur plusieurs sessions, je veux une gestion professionnelle des bugs avec index compact, dossier detaille par bug et preuves separees, afin de conserver une vraie tracabilite sans transformer shipglows_data/workflow/TEST_LOG.md ou shipglows_data/workflow/BUGS.md en fichiers ingerables."
confidence: high
risk_level: medium
security_impact: yes
docs_impact: yes
...
En tant qu'utilisatrice ShipGlows qui corrige des bugs avec des agents sur plusieurs sessions, je veux une gestion professionnelle des bugs avec index compact, dossier detaille par bug et preuves separees, afin de conserver une vraie tracabilite sans transformer `shipglows_data/workflow/TEST_LOG.md` ou `shipglows_data/workflow/BUGS.md` en fichiers ingerables.

## Minimal Behavior Contract

Quand un test manuel, une verification, un diagnostic navigateur, une correction ou une tentative de ship revele un bug, ShipGlows doit creer ou mettre a jour un bug identifiable par un ID stable, afficher une trace courte dans les index du projet, et conserver le detail complet dans un dossier dedie au bug. Les index doivent permettre de comprendre rapidement les bugs ouverts, bloques, a retester ou fermes; le dossier du bug doit contenir reproduction, attendu, observe, preuves redigees ou referencees, diagnostic, tentatives de correction, retests, statut et prochaine action. En cas d'information incomplete, le bug reste ouvert avec un statut explicite comme `needs-info` ou `needs-repro`, sans inventer les preuves manquantes ni fermer sur hypothese. L'edge case facile a rater est la preuve volumineuse ou sensible: elle doit etre redigee avant persistance et referencee par chemin, jamais collee brute dans `shipglows_data/workflow/TEST_LOG.md`, `shipglows_data/workflow/BUGS.md` ou le dossier bug.

## Success Behavior

- Preconditions: un projet utilise ShipGlows et peut contenir `shipglows_data/workflow/TEST_LOG.md`, `shipglows_data/workflow/BUGS.md`, `shipglows_data/workflow/bugs/`, `test-evidence/`, des skills ShipGlows et des docs publiques de skills.
...
- User/operator result: l'utilisatrice voit un identifiant stable `BUG-YYYY-MM-DD-NNN`, un `shipglows_data/workflow/BUGS.md` compact, un dossier `shipglows_data/workflow/bugs/BUG-ID.md`, les preuves sous `test-evidence/BUG-ID/` quand necessaire, un statut canonique et la prochaine commande claire.
- System effect: ShipGlows append ou met a jour les fichiers concernes sans supprimer l'historique existant; les gros logs, HAR, captures et dumps sont references par chemin apres redaction ou explicitement refuses si la redaction n'est pas prouvable.
- Success proof: ouvrir `shipglows_data/workflow/BUGS.md` donne l'etat global en moins d'une minute; ouvrir `shipglows_data/workflow/bugs/BUG-ID.md` donne assez de contexte pour qu'un agent frais reproduise, corrige, reteste ou refuse une fermeture dangereuse sans lire l'historique de conversation.
- Silent success: not allowed; toute creation, mise a jour, retest, blocage de ship ou fermeture de bug doit etre visible dans `shipglows_data/workflow/BUGS.md`, dans le dossier bug ou dans le rapport final du skill.

## Error Behavior

- Expected failures: bug deja existant, reproduction incomplete, preuve trop volumineuse, preuve sensible non redigee, bug non reproductible, plusieurs bugs similaires, fichier non modifiable, evidence fournie sans chemin, collision d'ID, conflit entre `shipglows_data/workflow/BUGS.md` et `shipglows_data/workflow/bugs/`, correction tentee sans retest, ship demande avec bug high/critical ouvert.
...
- System effect: aucun gros log inline; aucune duplication non controlee dans `shipglows_data/workflow/TEST_LOG.md`; aucun bug marque `closed` sans retest ou exception `closed-without-retest`; aucune preuve brute connue comme sensible ne doit etre persistee; aucun ancien contexte ne doit etre supprime.
...
`sg-test` a deja la bonne intention: transformer un resultat de test en memoire projet durable. Mais le modele actuel melange trop de responsabilites. `shipglows_data/workflow/TEST_LOG.md` risque de devenir un journal gigantesque si chaque echec contient reproduction, captures, logs, hypotheses, tentatives de fix et retests. `shipglows_data/workflow/BUGS.md` risque aussi de devenir illisible si chaque bug y accumule tout son historique.
...
Introduire une architecture de bug tracking ShipGlows en trois couches: `shipglows_data/workflow/TEST_LOG.md` reste un journal compact des scenarios testes, `shipglows_data/workflow/BUGS.md` devient un index compact des bugs, et `shipglows_data/workflow/bugs/BUG-ID.md` devient le dossier complet du bug. Les preuves volumineuses vivent dans `test-evidence/BUG-ID/` quand elles sont redigees et utiles, ou restent en reference externe si elles ne doivent pas etre copiees. Les skills `sg-test`, `sg-fix`, `sg-verify`, `sg-ship`, `sg-docs` et les docs doivent appliquer la meme separation de responsabilites, les memes statuts et les memes regles de preuve.

## Scope In

- Definir la structure projet standard pour bugs: `shipglows_data/workflow/BUGS.md`, `shipglows_data/workflow/bugs/`, `test-evidence/`.
- Transformer `shipglows_data/workflow/BUGS.md` en index compact plutot qu'en dossier complet.
- Creer un format de dossier bug individuel `shipglows_data/workflow/bugs/BUG-ID.md`.
...
- Definir la generation d'ID et la resolution de collisions entre `shipglows_data/workflow/BUGS.md`, `shipglows_data/workflow/bugs/` et plusieurs agents.
...
- `shipglows_data/workflow/TEST_LOG.md` et `shipglows_data/workflow/BUGS.md` restent des trackers operationnels courts, sans frontmatter obligatoire.
- `shipglows_data/workflow/bugs/BUG-ID.md` est un artefact durable ShipGlows et doit utiliser le frontmatter standard avec `artifact: bug_record`.
- `tools/shipglows_metadata_lint.py` doit accepter `artifact: bug_record` et peut linter `shipglows_data/workflow/bugs/*.md` quand le chemin est fourni ou quand un dossier `shipglows_data/workflow/bugs/` existe.
...
- Runtime: markdown, YAML frontmatter pour `shipglows_data/workflow/bugs/*.md`, conventions de nommage `BUG-YYYY-MM-DD-NNN`, relecture de fichiers avant ecriture.
- Document contracts: `PRODUCT.md` 1.1.0, `BUSINESS.md` 1.1.0, `BRANDING.md` 1.0.0, `GUIDELINES.md` 1.0.0, `shipglows_data/workflow/playbooks/spec-driven-workflow.md` 0.3.0.
...
- Metadata gaps: `shipglows_data/workflow/BUGS.md` and `shipglows_data/workflow/TEST_LOG.md` intentionally remain tracker files and should not be migrated to artifact frontmatter.
- Fresh external docs: fresh-docs not needed, because the change is local to ShipGlows markdown conventions, Python standard library linting, and skill instructions. No framework, SDK, service API, auth provider, build system, migration engine or external integration behavior determines this spec.

## Invariants

- `shipglows_data/workflow/TEST_LOG.md` answers: what was tested, when, with what result, and which bug or next step followed.
- `shipglows_data/workflow/BUGS.md` answers: what bugs are open, blocked, duplicate, closed, severity, owner/status if known, last tested date, and link to the detailed dossier.
- `shipglows_data/workflow/bugs/BUG-ID.md` answers: how the bug reproduces, what evidence exists, what was tried, what changed, and how retests behaved.
...
- Existing `shipglows_data/workflow/BUGS.md` content remains valid legacy content; new or retested bugs should use the new compact index plus dossier model.
...
- Before assigning an ID, the skill must re-read `shipglows_data/workflow/BUGS.md` and list `shipglows_data/workflow/bugs/BUG-YYYY-MM-DD-*.md`, then pick the next numeric suffix greater than every suffix found in either place.
...
- Regression impact: existing projects with simple `shipglows_data/workflow/BUGS.md` should continue working; the new structure is additive and applies when new bugs are opened or retested.
- Public documentation impact: the website skill pages and README must not describe old `shipglows_data/workflow/BUGS.md` behavior after the new dossier model lands.
- Operational consequence: no global registry in `${SHIPGLOWS_DATA_DIR:-$HOME/shipglows_data}`; bug records belong to the project repo where the behavior exists.

## Documentation Coherence

- `skills/sg-test/SKILL.md` must explain compact `shipglows_data/workflow/TEST_LOG.md`, compact `shipglows_data/workflow/BUGS.md`, per-bug dossiers, ID generation, status lifecycle, evidence storage, redaction, and retest behavior.
...
- `README.md` and `shipglows_data/workflow/playbooks/spec-driven-workflow.md` must mention the professional bug loop.
...
- ID collision: re-read both `shipglows_data/workflow/BUGS.md` and `shipglows_data/workflow/bugs/` immediately before writing; if the chosen ID now exists, increment and retry once. If still conflicting, stop and report the collision.
- `shipglows_data/workflow/BUGS.md` references a missing dossier: keep index entry, set or report `needs-info`, and do not erase the index.
- Dossier exists without `shipglows_data/workflow/BUGS.md` entry: add a compact index pointer if the dossier frontmatter is valid; otherwise report metadata gap.
...
- Multiple projects: bug artifacts belong to the project repo where the behavior exists, not global `shipglows_data`.
...
- [ ] Task 1: Add the bug dossier artifact template
  ...
  - Action: Create a ShipGlows artifact template for `shipglows_data/workflow/bugs/BUG-ID.md` with frontmatter, reproduction, expected, observed, evidence, diagnosis notes, fix attempts, retest history, related shipglows_data/workflow/bugs/artifacts, status lifecycle notes, redaction status, and closure criteria.
...
- [ ] Task 2: Add bug record metadata lint support
  - File: `tools/shipglows_metadata_lint.py`
  - Action: Accept `artifact: bug_record`, require the bug-specific frontmatter fields needed by the template, and include `shipglows_data/workflow/bugs/` in default targets only when that directory exists or when the path is explicitly passed.
  - User story link: Keeps detailed bug dossiers structured and reviewable by fresh agents.
  - Depends on: Task 1
  - Validate with: `SHIPGLOWS_ROOT="${SHIPGLOWS_ROOT:-$HOME/shipglows}" "$SHIPGLOWS_ROOT/tools/shipglows_metadata_lint.py" templates/bug_record.md`
  - Notes: Do not lint `shipglows_data/workflow/BUGS.md` or `shipglows_data/workflow/TEST_LOG.md` as artifacts.

- [ ] Task 3: Define compact test and bug tracking doctrine in `sg-test`
  - File: `skills/sg-test/SKILL.md`
  - Action: Replace the current full `shipglows_data/workflow/BUGS.md` append format with compact `shipglows_data/workflow/BUGS.md` index rules, `shipglows_data/workflow/bugs/BUG-ID.md` dossier rules, `test-evidence/BUG-ID/` rules, canonical statuses, ID generation, duplicate detection and evidence redaction.
  - User story link: Prevents `shipglows_data/workflow/TEST_LOG.md` and `shipglows_data/workflow/BUGS.md` from becoming unbounded investigation dumps.
...
- [ ] Task 4: Update `sg-test` retest behavior
  - File: `skills/sg-test/SKILL.md`
  - Action: Specify that `--retest BUG-ID` reads `shipglows_data/workflow/bugs/BUG-ID.md`, appends retest history there, writes only a short `shipglows_data/workflow/TEST_LOG.md` pointer, and moves status only through allowed transitions.
...
- [ ] Task 5: Update `sg-fix` bug dossier intake
  ...
  - Action: When invoked with `BUG-ID` or a matching bug title, read `shipglows_data/workflow/BUGS.md` and `shipglows_data/workflow/bugs/BUG-ID.md`, reconstruct the bug story from the dossier, append diagnosis notes and fix attempts, and refuse closure without retest evidence.
...
- [ ] Task 6: Update verification gate behavior
  ...
  - Action: Require `sg-verify` to inspect linked `shipglows_data/workflow/BUGS.md` and `shipglows_data/workflow/bugs/*.md` for the verified scope, report open high/critical bugs, and block optimistic closure when bug state contradicts the user story.
  - User story link: Prevents verification from claiming success while known core bugs remain unresolved.
  - Depends on: Task 5
  - Validate with: `rg -n "shipglows_data/workflow/BUGS.md|shipglows_data/workflow/bugs/|open bug|fixed-pending-verify|high|critical|blocks ship" skills/sg-verify/SKILL.md`
...
- [ ] Task 7: Update shipping gate behavior
  ...
  - Depends on: Task 6
  - Validate with: `rg -n "shipglows_data/workflow/BUGS.md|shipglows_data/workflow/bugs/|high|critical|blocked|partial-risk|bug" skills/sg-ship/SKILL.md`
...
- [ ] Task 8: Update documentation coherence skill
  ...
  - Action: Teach docs audit/update mode to recognize the professional bug model, verify docs mention `shipglows_data/workflow/TEST_LOG.md`, compact `shipglows_data/workflow/BUGS.md`, per-bug dossiers and evidence redaction, and preserve the rule that trackers do not need frontmatter.
  - User story link: Keeps the documentation layer aligned with the new workflow.
  - Depends on: Tasks 3-7
  - Validate with: `rg -n "shipglows_data/workflow/BUGS.md|shipglows_data/workflow/TEST_LOG.md|bug dossier|test-evidence|tracker" skills/sg-docs/SKILL.md`
...
- [ ] Task 9: Update help command coverage
  ...
  - Action: Add the professional bug loop and the roles of `shipglows_data/workflow/TEST_LOG.md`, `shipglows_data/workflow/BUGS.md`, `shipglows_data/workflow/bugs/BUG-ID.md`, and `test-evidence/BUG-ID/`.
  - User story link: A fresh user can discover the workflow without reading implementation skills.
  - Depends on: Tasks 3-8
  - Validate with: `rg -n "bug dossier|shipglows_data/workflow/TEST_LOG.md|shipglows_data/workflow/BUGS.md|test-evidence|sg-test --retest" skills/sg-help/SKILL.md`
...
- [ ] Task 10: Update `sg-test` README
  ...
  - Validate with: `rg -n "bug dossier|test-evidence|shipglows_data/workflow/BUGS.md|shipglows_data/workflow/TEST_LOG.md|redact" skills/sg-test/README.md`
  - Notes: Avoid promising external tracker sync.

- [ ] Task 11: Update workflow doctrine
  - File: `shipglows_data/workflow/playbooks/spec-driven-workflow.md`
  ...
  - Validate with: `rg -n "Professional Bug|bug dossier|test-evidence|sg-test --retest|closed-without-retest" shipglows_data/workflow/playbooks/spec-driven-workflow.md`
...
- [ ] Task 12: Update README
  ...
  - User story link: A fresh user understands that ShipGlows now preserves bug traceability.
  - Depends on: Task 11
  - Validate with: `rg -n "bug|shipglows_data/workflow/BUGS.md|shipglows_data/workflow/TEST_LOG.md|test-evidence|shipglows_data/workflow/bugs/" README.md`
...
- [ ] Task 13: Update public `sg-test` skill page
  ...
  - Action: Align the public page with compact `shipglows_data/workflow/TEST_LOG.md`, compact `shipglows_data/workflow/BUGS.md`, per-bug dossiers and redacted evidence directories.
  - User story link: Prevents public docs from teaching the old bug model.
  - Depends on: Tasks 10-12
  - Validate with: `rg -n "bug dossier|test-evidence|shipglows_data/workflow/BUGS.md|shipglows_data/workflow/TEST_LOG.md" site/src/content/skills/sg-test.md`
...
- [ ] Task 15: Update public verification skill page
  ...
  - Depends on: Task 14
  - Validate with: `rg -n "bug|shipglows_data/workflow/BUGS.md|blocks|verification" site/src/content/skills/sg-verify.md`
...
- [ ] Task 17: Update public docs skill page
  ...
  - Depends on: Task 16
  - Validate with: `rg -n "bug|documentation coherence|shipglows_data/workflow/BUGS.md|shipglows_data/workflow/TEST_LOG.md" site/src/content/skills/sg-docs.md`
...
- [ ] CA 1: Given a failed `sg-test` scenario, when the result is logged, then `shipglows_data/workflow/TEST_LOG.md` contains a short scenario entry with a pointer to `BUG-ID` and `shipglows_data/workflow/bugs/BUG-ID.md` contains the detailed reproduction, expected behavior, observed behavior, evidence, current status and next step.
- [ ] CA 2: Given a new bug is created, when `shipglows_data/workflow/BUGS.md` and `shipglows_data/workflow/bugs/` already contain bugs for the same UTC date, then the new ID uses a suffix greater than every existing suffix and does not overwrite an existing dossier.
...
- [ ] CA 9: Given an existing legacy `shipglows_data/workflow/BUGS.md`, when a new bug is created, then the legacy content is preserved and the new index entry stays compact with a link to the dossier.
- [ ] CA 10: Given `tools/shipglows_metadata_lint.py` is run on `templates/bug_record.md`, then the template passes metadata lint with `artifact: bug_record`.
...
- [ ] CA 13: Given docs are updated, when README, workflow docs, help, public skill pages and changelog are searched, then they no longer describe `shipglows_data/workflow/BUGS.md` as the full bug dossier location.
- [ ] CA 14: Given a fresh agent opens only `shipglows_data/workflow/BUGS.md` and `shipglows_data/workflow/bugs/BUG-ID.md`, when asked to continue work, then it can identify current status, reproduction, attempted fixes, evidence, redaction state and next step without reading chat history.

## Test Strategy

- Unit: Add or update targeted metadata-linter coverage only if the repo already has a test harness for `tools/shipglows_metadata_lint.py`; otherwise validate by running the linter directly on the new template.
- Integration: Run metadata lint against `templates/bug_record.md`; if sample fixtures are added, also lint a sample `shipglows_data/workflow/bugs/BUG-ID.md`.
- Manual: Simulate a failed `sg-test` in a disposable project or fixture, verify compact `shipglows_data/workflow/TEST_LOG.md`, compact `shipglows_data/workflow/BUGS.md`, `shipglows_data/workflow/bugs/BUG-ID.md`, redacted evidence references, then simulate `sg-fix BUG-ID`, `sg-test --retest BUG-ID`, `sg-verify`, and a quick `sg-ship` report with an open high/critical bug.
- Regression: Verify existing `sg-test` pass/blocked/not-run paths still keep `shipglows_data/workflow/TEST_LOG.md` compact and do not create bug dossiers unnecessarily.
...
- Migration risk: low to medium, because legacy `shipglows_data/workflow/BUGS.md` files may already contain details. Mitigation: leave old content valid and apply the new structure only to new or retested bugs unless a project explicitly requests migration.
...
- Concurrency risk: low to medium, because multiple agents could assign IDs. Mitigation: re-read `shipglows_data/workflow/BUGS.md` and `shipglows_data/workflow/bugs/` just before writing and retry on collision; stop on repeated conflict.

## Execution Notes

- Read first: `skills/sg-test/SKILL.md`, `skills/sg-fix/SKILL.md`, `skills/sg-verify/SKILL.md`, `skills/sg-ship/SKILL.md`, `skills/sg-docs/SKILL.md`, `templates/spec.md`, `tools/shipglows_metadata_lint.py`.
...
- Packages: do not add runtime packages. `tools/shipglows_metadata_lint.py` must remain Python standard library only.
- Patterns to follow: additive markdown edits, re-read before editing trackers, explicit status transitions, explicit evidence limits, no frontmatter for `shipglows_data/workflow/TEST_LOG.md` or `shipglows_data/workflow/BUGS.md`, ShipGlows-owned paths from `${SHIPGLOWS_ROOT:-$HOME/shipglows}`.
- Data flow: observation or failed test -> compact `shipglows_data/workflow/TEST_LOG.md` entry -> compact `shipglows_data/workflow/BUGS.md` index pointer -> detailed `shipglows_data/workflow/bugs/BUG-ID.md` dossier -> optional redacted evidence under `test-evidence/BUG-ID/` -> fix attempt -> retest -> verify -> ship risk report.
...
- Validate with: metadata lint for the new template, `rg` checks listed in tasks, manual dry-run against a fixture or temporary project, and final `tools/shipglows_metadata_lint.py specs/professional-bug-management.md templates/bug_record.md` after implementation.
