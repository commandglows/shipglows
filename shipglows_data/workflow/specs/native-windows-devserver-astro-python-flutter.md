...
linked_systems:
  ...
  - "/home/claude/shipglows_app/site/src/data/installPages.ts"
  - "/home/claude/shipglows_app/site/src/pages/install.astro"
  - "/home/claude/shipglows_app/site/src/pages/fr/install.astro"
...
- Coding agents and preferred stacks: Windows full proposes missing Codex, Claude Code, OpenCode, Kilo (`kilocode` compatibility) and Gemini once, installs only accepted exact versions, then prepares Dart/Flutter, Playwright, Firebase, Convex and Clerk MCP plus the official read-only GitHub MCP without reading credentials or starting authentication. Gemini uses its official user-scope CLI and verifies local settings without connecting. Existing JSON/JSONC may remain explicitly pending to preserve comments and secrets. Firebase, FlutterFire, Convex, Vercel, Supabase and Clerk CLIs are manifest-driven; Clerk SDK/init/linking and Android-native dependencies remain project-driven.
...
- Fresh external docs verdict: `fresh-docs checked` on 2026-08-15 against:
  ...
  - Clerk CLI and MCP: https://clerk.com/docs/cli and https://clerk.com/docs/guides/ai/mcp/clerk-mcp-server
  - GitHub MCP Server: https://github.com/github/github-mcp-server and https://docs.github.com/en/copilot/how-tos/provide-context/use-mcp-in-your-ide/set-up-the-github-mcp-server
  - Gemini CLI installation, MCP and global context: https://github.com/google-gemini/gemini-cli/blob/main/docs/get-started/installation.mdx, https://geminicli.com/docs/tools/mcp-server/ and https://geminicli.com/docs/tools/memory/
...
| 2026-08-16 07:56:20 UTC | sg-content | GPT-5 Codex | Aligned the canonical Windows installer introduction and the declared ShipGlows public installer surfaces with the verified Flutter Android/Windows toolchain, then corrected the installer page locale switch found during browser proof. | Regression-first bilingual copy, handoff, parity and locale-route assertions pass; the full Windows suite, full Astro tests/check/build, and isolated browser proof across EN/FR installer routes pass with no console warnings or errors. No deployment was performed. | Publish the two dedicated branches, integrate the ShipGlows source branch before the site branch, then deploy through the normal site release path. |
| 2026-08-16 11:15:54 UTC | sg-development | GPT-5 Codex | Extended Windows full from SDK preparation to preferred-stack and agent readiness: grouped exact-version agent installation, bounded Firebase/FlutterFire/Convex/Vercel/Supabase detection, official Firebase/Convex MCP definitions, truthful per-agent MCP reporting, and read-only Developer Mode guidance. | Regression-first mobile fixtures, PowerShell 5.1 parsing, the full Windows contract, 27 public installer tests, bootstrap parity, Astro check and production build pass. No real package install, authentication, licence flow, registry change, deployment or live runtime update was executed. | Request separate authority to publish both validated branches and run the remote-branch full installer on Shadow. |
