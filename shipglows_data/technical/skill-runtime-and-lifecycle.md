...
- Repository-local `task_branch_policy` and `worktree_policy` live in `shipglows_data/technical/guidelines.md`, default independently to `forbidden`, and are resolved immediately before creation by `tools/project_git_policy.py`. `shipglows context` exposes their effective values and source; inventory and proven-integrated cleanup of existing artifacts remain available when creation is forbidden.
