...
No ShipGlows server install is required for SSH Access itself. The optional local SSH config below only avoids host-key friction with ephemeral Blacksmith VMs:
...
```bash
python3 tools/shipglows_metadata_lint.py shipglows_data/technical/blacksmith.md
```
