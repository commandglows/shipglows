...
evidence:
  ...
  - "The 2026-08-23 native Windows corpus surface validates the Git origin, removes the conflicting public plugin through Codex, and verifies live skill junctions to the contributor checkout."
...
- Native Windows `full` defaults to the project-development `runtime` surface. The explicit `corpus` surface (interactive option 3, `-InstallSurface corpus`, or compatible `SHIPGLOWS_INSTALL_COMPONENTS=all|skills|corpus`) clones the complete branch namespace or validates the editable ShipGlows checkout at `%USERPROFILE%\ShipGlows\shipglows`, removes enabled public ShipGlows plugins through the Codex CLI, and links the public Codex catalogue to that checkout. Existing repositories are never updated or switched implicitly; origin and required-file validation must pass first.
