...
- Native Windows `full` is a project-development runtime, not a contributor checkout of ShipGlows. It may clone operator-selected project repositories under `%USERPROFILE%\ShipGlows`, while the editable ShipGlows contributor clone is a separate Git checkout at `%USERPROFILE%\ShipGlows\shipglows` governed by `skills/references/windows-bootstrap-development-workflow.md`.
