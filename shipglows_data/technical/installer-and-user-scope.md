...
evidence:
  ...
  - "Native Windows proposes the optional s p private-data flow after installation; it requests consent before GitHub login or clone and keeps the selected repository outside the project workspace."
...
- Native Windows keeps internal source and command wrappers under `%USERPROFILE%\.shipglows\runtime`; the parent `.shipglows` directory stays hidden and its private-data working tree is `%USERPROFILE%\.shipglows\data`. The installed completion message proposes `s p`: it does nothing until an operator explicitly chooses a private GitHub repository, then GitHub CLI alone performs browser authentication and the repository is preflighted for Windows-incompatible paths before cloning. User projects live directly under `%USERPROFILE%\ShipGlows`; private data is never scanned, registered, started, or synchronized by the DevServer. Migration removes only legacy `bin`, `cli`, and `local` runtime directories and removes the old visible `workspace` directory only when it is empty.
