...
- A validated ShipGlows-managed Flutter SDK reconverges its `bin` directory into both the persistent user `PATH` and the active installer process on every full rerun. The environment report keeps Visual Studio Desktop C++ readiness separate and reports the aggregate Flutter Windows desktop toolchain as ready only when Flutter/Dart, that workload, and Developer Mode all validate. A momentary missing Dart executable during a cache replacement requires bounded command revalidation before corruption is reported.
