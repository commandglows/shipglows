...
- The isolated machine toolbox owns `[settings] lockfile = true` with
  `lockfile_platforms = ["windows-x64"]`. After converging its exact provider
  coordinates, the installer runs `mise lock --platform windows-x64` so Aqua
  artifacts such as Supabase and Auth0 carry platform-specific integrity
  metadata in `mise.lock`. A lock refresh warning does not loosen versions or
  authenticate providers: the exact `mise.toml` pins remain authoritative.
...
- The packaged environment surface also carries the closed Windows provider and composable adapter modules. A Tauri desktop apply can propose only trusted mise acquisition or isolated Rust 1.97.1 installation after explicit digest consent; it never installs MSVC, Windows SDK or WebView2 silently, changes Windows policy, executes repository arguments, installs project dependencies, or mutates project manifests. Runtime `cargo`/`rustc`/`rustup` wrappers preserve the isolated mise settings for fresh processes. Fixture and archive proofs are complete, but live installed-runtime convergence and CommunityGlows apply remain separate pending execution gates.
