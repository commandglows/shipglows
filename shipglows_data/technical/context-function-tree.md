...
Ce document sert de point d'entree rapide pour comprendre la structure fonctionnelle de ShipGlows sans relire tout `lib.sh`.
...
- `cli/windows/ShipGlows.DevServer.psm1`: detection Astro/Vite/extensions navigateur/plugins Obsidian/Python/Flutter, registre, ports, dependances, readiness, synchronisation explicite et processus Windows.
...
install-devserver.ps1
  ...
  -> Disable-SgBlockedPowerShellShim
  -> Install-SgApplicationCommandWrapper
  -> Install-SgAgentShortcut (c/co/cor/oc/kc)

shipglows-devserver.ps1
  ...
  -> ShipGlows.DevServer.psm1
     -> Get-SgWorkspaceProjectCandidates (memory/persistent cache)
        -> Find-SgWorkspaceProjectCandidates (single bounded linear scan)
           -> Get-SgObsidianPluginDescriptor (native evidence before generic Vite)
        -> Write-SgProjectIndex (validated atomic replacement)
     -> Reconcile-SgRegistry (live-state authority)
     -> Start-SgProject
        -> Get-SgLaunchSpec (declared Obsidian watch script, portless)
        -> Wait-SgObsidianPluginReady (fresh artifact evidence)
        -> Sync-SgObsidianPluginArtifacts (explicit vault, verified copy)
...
menu frontends
  -> cli/shipglows_devserver_gum.sh::_gum_run_menu
  -> cli/shipglows_devserver_gum.sh::_gum_run_nested_menu
  -> cli/shipglows_devserver_bash.sh::_bash_run_menu
  -> cli/shipglows_devserver_bash.sh::_bash_run_nested_menu
...
environment discovery
  ...
  -> resolve_project_paths_into (environment root + application launch path)
  -> resolve_project_path_into
  -> resolve_project_launch_path_into
  -> resolve_project_path (compatibility stdout wrapper)
  -> resolve_environment_pm2_name_into (bounded legacy PM2 identity migration)
...
2. `lib.sh`
3. `config.sh`
4. `cli/shipglows_devserver_gum.sh` or `cli/shipglows_devserver_bash.sh`
...
- Le frontend de menu est delegue a `cli/shipglows_devserver_gum.sh` ou `cli/shipglows_devserver_bash.sh`, non detailles ici.
