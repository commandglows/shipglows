...
| `shipglows_data/technical/architecture.md` | Global structure and boundaries | Store current architecture, not every historical debate |
| `shipglows_data/technical/guidelines.md` | General technical doctrine | Store durable rules and anti-patterns |
| `shipglows_data/workflow/specs/*.md` | Chantiers and run history | Store task-specific decisions and evidence |
| `shipglows_data/technical/*.md` | Subsystem operational context | Store code-proximate behavior and maintenance rules |
| `shipglows_data/technical/decisions/*.md` | ADR-style decisions | Store durable cross-cutting choices such as governance layout |

## Entrypoints

- `templates/decision_record.md`: start here when a durable ADR-style decision artifact is needed.
- `shipglows_data/technical/architecture.md`: update when a decision changes global structure, system boundaries, or architectural invariants.
...
- Use `shipglows_data/technical/architecture.md` when the accepted decision changes global structure.
- Use `shipglows_data/technical/guidelines.md` when the accepted decision becomes a general engineering rule.
...
```bash
python3 tools/shipglows_metadata_lint.py templates/decision_record.md shipglows_data/technical/decisions shipglows_data/technical/architecture.md shipglows_data/technical/guidelines.md shipglows_data/workflow/specs
rg -n "decision|rationale|consequences|templates/decision_record.md" shipglows_data/technical/decisions.md
```

## Reader Checklist

- A spec records a decision that should outlive the chantier -> propose a decision record or update the relevant contract.
- `shipglows_data/technical/architecture.md` or `shipglows_data/technical/guidelines.md` changed -> check affected specs and technical docs for version drift.
