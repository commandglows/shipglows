...
evidence:
  ...
  - "Local menu headers refreshed to match server ShipGlows DevServer treatment."
...
This doc covers the local tools that connect a workstation to a remote ShipGlows server: app tunnels, saved SSH connection state, password-to-key promotion, remote PM2 and Flutter Web `tmux` port discovery, `shipglows-mcp-login` for remote Codex MCP OAuth callbacks, `shipglows-clerk-login` for remote Clerk CLI OAuth callbacks, `shipglows-blacksmith-login` for remote Blacksmith CLI OAuth callbacks, `shipglows-turso-login` for remote Turso CLI login, and `shipglows-turso-ssh` for Turso CLI auth transfer and schema proof.

Blacksmith SSH Access is intentionally separate from these OAuth callback tunnels. It connects a local terminal directly to an ephemeral Blacksmith runner for a live GitHub Actions job; it does not use `shipglows-blacksmith-login` and does not require a ShipGlows server install.
...
| `local/install.sh`, `local/install_local.ps1`, `install-shipglows.ps1` | Local installer scripts | Keep Termux, Linux, macOS, WSL, and Windows assumptions explicit |
...
- `shipglows-mcp-login <provider|all>`: launches remote Codex MCP login and opens a temporary callback tunnel.
- `shipglows-clerk-login`: launches remote `clerk auth login`, opens a temporary callback tunnel, and verifies with `clerk whoami`.
- `shipglows-blacksmith-login`: launches remote `blacksmith auth login` and opens a temporary callback tunnel.
- `shipglows-turso-login`: launches remote `turso auth login --headless` by default, opens/prints the auth URL, accepts the browser-provided JWT/token when Turso shows one, stores it through the official remote CLI, then verifies `turso auth whoami`.
...
- `shipglows-turso-ssh [db-name]`: copies local Turso CLI config to the remote server, verifies `turso auth whoami`, and optionally checks ContentFlow tables.
...
```text
shipglows-turso-ssh
  -> load saved ShipGlows SSH target and optional identity
...
- Managed tunnel stop logic should select ShipGlows-owned tunnels, not broad process patterns.
...
- Raw SSH process listing is debug-only operator output via `SHIPGLOWS_DEBUG=1`.
- Active Flutter Web `tmux` ports are discovered from the server-side
  `SHIPGLOWS_FLUTTER_WEB_SESSIONS_FILE` registry and included only when the
  recorded `tmux` session still exists.
- OAuth tokens remain owned by Codex and the provider; ShipGlows only routes the callback.
- Clerk auth tokens remain owned by Clerk CLI on the remote server; ShipGlows
  only verifies auth through `clerk whoami`.
- Blacksmith auth tokens remain owned by Blacksmith CLI on the remote server;
  ShipGlows only checks credentials-file presence.
- Turso tokens remain owned by the Turso CLI config. `shipglows-turso-ssh`
  copies the config only on explicit invocation and never logs token contents.
  `shipglows-turso-login` verifies auth with `turso auth whoami` only.
...
- Key/agent SSH calls run in batch mode so menu scans fail visibly instead of
  ...
  control socket for eight hours; the password is never saved by ShipGlows.
- The startup session scan is operator feedback only; set `SHIPGLOWS_NO_ANIMATION=1` to disable the animated TTY loader.
- Local menu screens should use the shared local header treatment:
  `ShipGlows DevServer` in yellow, padded boxed header, then the screen title.
...
rg -n "validate_connection_target|validate_identity_file|check_local_port_free|parse_mcp_oauth_port_from_text|parse_clerk_oauth_port_from_text|shipglows-clerk-login|shipglows-turso-login|shipglows-turso-ssh" local/
```
