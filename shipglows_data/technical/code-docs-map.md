...
| `tools/skill_activation_budget.py`, `tools/test_skill_activation_budget.py`, registry `activation_profiles` | Reference-activation accounting | `skills/references/skill-context-budget.md` | `skills/references/skill-instruction-layering.md`, `shipglows_data/technical/skill-runtime-and-lifecycle.md` | `python3 -m unittest tools.test_skill_activation_budget`; `python3 tools/skill_activation_budget.py --format text` | Pilot body/baseline/gate paths, unique-token accounting, profile preflight, or profile generalization changes |
...
| `skills/references/design-inspiration-library.md`, `skills/references/design-inspiration/**`, `tools/capture_design_inspiration.py`, `tools/capture_design_inspiration_playwright.js`, `tools/test_capture_design_inspiration.py`, `tools/fixtures/design-inspiration/**` | Private design inspiration library | `shipglows_data/technical/skill-runtime-and-lifecycle.md` | `README.md`, `skills/references/private-data-repo-contract.md`, `shipglows_data/workflow/playbooks/design-inspiration-library-server-migration-playbook.md` | `python3 -m unittest tools.test_capture_design_inspiration`; `node --check tools/capture_design_inspiration_playwright.js`; synthetic `--fixture --no-network` capture into a temporary directory; shared-runtime local HTTP smoke; metadata lint; targeted Inspiration Gate `rg` checks | Private/public storage boundary, verified private-origin/LFS synchronization, server restore, Git LFS/rotation policy, taxonomy and bounded search index, global Playwright discovery/retry recovery, tall-page WebP bounding, static-only capture contract, capture bundle/schema, extraction, browser/image pipeline, checksums/dedupe, rights status, or design/copy skill consumption changes |
