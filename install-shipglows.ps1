...
function Extract-ShipglowsWindowsFiles([string]$ArchivePath, [string]$DestinationPath, [bool]$FullMode) {
    ...
    if ($FullMode) {
        ...
        $entries += @($archiveEntries | Where-Object { $_ -match '^[^/]+/cli/(?:private_data\.py|environment/(?:__init__\.py|core\.py|mise_backend\.py|shipglows_environment\.py|schemas/shipglows-environment-v1\.schema\.json))$' })
        $entries += @($archiveEntries | Where-Object { $_ -match '^[^/]+/shipglows-version\.json$' })
        if ($entries.Count -ne 27) { Fail 'The ShipGlows archive is missing native Windows DevServer, runtime status, version metadata, ShipGlows command, build-artifact helper, managed PowerShell runtime, MCP catalog, developer corpus channel, project catalogue refresher, Flutter supervisor, installer engine/UI, authentication, agent instructions, environment control-plane, or private-data control-plane files.' }
    }
