...
function Extract-ShipglowsWindowsFiles([string]$ArchivePath, [string]$DestinationPath, [bool]$FullMode) {
    ...
    if ($FullMode) {
        ...
        if ($entries.Count -ne 37) { Fail 'The ShipGlows archive is missing the native CLI launcher, Windows DevServer, runtime status, version metadata, ShipGlows command, build-artifact helper, Extension/Obsidian Lab, managed PowerShell runtime, MCP catalog, WSL/Turso bootstrap, developer corpus channel, project catalogue refresher, Flutter supervisor, installer engine/UI, authentication, agent instructions, provider, environment control-plane, or private-data control-plane files.' }
    }
...
try {
    ...
    if ($InstallMode -eq 'local') {
        ...
        foreach ($launcherModule in @('ShipGlows.CliLauncher.cs','ShipGlows.DevServer.psm1','ShipGlows.RuntimeStatus.psm1','ShipGlows.FlutterSupervisor.ps1','ShipGlows.ProjectCatalogRefresh.ps1','ShipGlows.Auth.psm1','ShipGlows.MobileToolchain.psm1','ShipGlows.BuildArtifacts.psm1','shipglows-build-artifacts.ps1','ShipGlows.McpCatalog.json','ShipGlows.ExtensionLab.js','ShipGlows.ObsidianLab.js','ShipGlows.PowerShellRuntime.psm1','ShipGlows.PowerShellRuntime.json','ShipGlows.PowerShellBootstrap.ps1','shipglows.ps1')) { Copy-Item -LiteralPath (Join-Path $stagedWindows $launcherModule) -Destination (Join-Path $stagedBin $launcherModule) }
