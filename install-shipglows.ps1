...
function Resolve-SgInstallSurface {
    param(
        [string]$RequestedSurface,
        [string]$InteractiveSurface,
        [Parameter(Mandatory=$true)][string]$InstallMode
    )

    $surface = if ($RequestedSurface) { $RequestedSurface.Trim().ToLowerInvariant() } elseif ($InteractiveSurface) { $InteractiveSurface } else { 'runtime' }
    switch ($surface) {
        { $_ -in @('runtime', 'cli') } { return 'runtime' }
        { $_ -in @('maintainer', 'developer', 'contributor') } {
            if ($InstallMode -ne 'full') { throw 'The maintainer surface requires InstallMode full.' }
            return 'maintainer'
        }
        { $_ -in @('skills', 'corpus') } {
            throw 'Native Windows uses the public Codex plugin for the public skill corpus. Use InstallSurface runtime for ordinary work or InstallSurface maintainer for an editable ShipGlows owner checkout.'
        }
        default { throw 'InstallSurface must be runtime or maintainer.' }
    }
}
function Select-WindowsInstallMode {
    ...
    Write-Host '  3) ShipGlows maintainer workstation (editable source)'
    ...
    while ($true) {
        ...
        switch ($choice.Trim()) {
            ...
            '1' { return 'local' }
            '2' { return 'full' }
            '3' { $script:InteractiveInstallSurface = 'maintainer'; return 'full' }
            '0' { Fail 'Installation cancelled.' }
            default { Write-Warn 'Enter 1 for SSH tunnel, 2 for Local DevServer, 3 for ShipGlows maintainer setup, or 0 to cancel.' }
        }
    }
}
...
if ($InstallSurface -eq 'runtime' -and $requestedComponents -in @('all', 'skills', 'corpus')) {
    Write-Warn 'Public component selection does not activate the private maintainer channel. Codex users keep the public plugin unless InstallSurface maintainer is selected explicitly.'
}
...
function Extract-ShipglowsWindowsFiles([string]$ArchivePath, [string]$DestinationPath, [bool]$FullMode) {
    ...
    if ($FullMode) {
        ...
        if ($entries.Count -ne 35) { Fail 'The ShipGlows archive is missing native Windows DevServer, runtime status, version metadata, ShipGlows command, build-artifact helper, Extension Lab, managed PowerShell runtime, MCP catalog, WSL/Turso bootstrap, developer corpus channel, project catalogue refresher, Flutter supervisor, installer engine/UI, authentication, agent instructions, provider, environment control-plane, or private-data control-plane files.' }
    }
...
try {
    ...
    if ($InstallMode -eq 'local') {
        ...
        $managedRelativePaths = @($windowsFiles | ForEach-Object { "cli/windows/$_" }) + @($pythonFiles | ForEach-Object { "cli/environment/$_" }) + @('cli/private_data.py','cli/environment/schemas/shipglows-environment-v1.schema.json','cli/install-turso-cloud.sh','shipglows-version.json','private_data.py') + @('bin/ShipGlows.DevServer.psm1','bin/ShipGlows.RuntimeStatus.psm1','bin/ShipGlows.FlutterSupervisor.ps1','bin/ShipGlows.ProjectCatalogRefresh.ps1','bin/ShipGlows.Auth.psm1','bin/ShipGlows.MobileToolchain.psm1','bin/ShipGlows.BuildArtifacts.psm1','bin/shipglows-build-artifacts.ps1','bin/ShipGlows.ExtensionLab.js','bin/ShipGlows.PowerShellRuntime.psm1','bin/ShipGlows.PowerShellRuntime.json','bin/ShipGlows.PowerShellBootstrap.ps1','bin/shipglows-devserver.ps1','bin/shipglows.ps1')
        ...
        $tursoInstallerSource = Join-Path (Split-Path $windowsCandidates[0].FullName -Parent) 'install-turso-cloud.sh'
        if (-not (Test-Path -LiteralPath $tursoInstallerSource -PathType Leaf)) { Fail 'Bundled Turso Cloud installer was not found in the extracted payload.' }
        Copy-Item -LiteralPath $tursoInstallerSource -Destination (Join-Path $stagedCli 'install-turso-cloud.sh')
        Assert-EnvironmentPackage -EnvironmentDirectory $stagedEnvironment
        foreach ($launcherModule in @('ShipGlows.DevServer.psm1','ShipGlows.RuntimeStatus.psm1','ShipGlows.FlutterSupervisor.ps1','ShipGlows.ProjectCatalogRefresh.ps1','ShipGlows.Auth.psm1','ShipGlows.MobileToolchain.psm1','ShipGlows.BuildArtifacts.psm1','shipglows-build-artifacts.ps1','ShipGlows.McpCatalog.json','ShipGlows.ExtensionLab.js','ShipGlows.PowerShellRuntime.psm1','ShipGlows.PowerShellRuntime.json','ShipGlows.PowerShellBootstrap.ps1','shipglows.ps1')) { Copy-Item -LiteralPath (Join-Path $stagedWindows $launcherModule) -Destination (Join-Path $stagedBin $launcherModule) }
    ...
    if ($InstallSurface -eq 'maintainer') {
        ...
        Write-Info 'Codex now uses the live ShipGlows maintainer skill channel. Start a new Codex session to load it.'
    }
