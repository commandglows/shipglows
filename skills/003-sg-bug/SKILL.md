...
Public label: `sg-bug`. Load `$SHIPGLOWS_ROOT/skills/references/intent-to-outcome-autonomy.md` before clarification or bug-lifecycle routing. Resolve `project -> product -> surface -> feature`, gather reproducible evidence autonomously, and retain ownership through fix, retest, verification, documentation coherence, and authorized closure/ship.
