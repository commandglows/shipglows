...
- `SSRP-019 visual closure card`: a successful closure uses the five ordered blocks `✨ RÉSULTAT`, `🧪 PREUVES`, `📖 DOCUMENTATION`, `📰 ÉDITORIAL`, and `📦 LIVRAISON`; proof, documentation, and editorial content each occupy one line with ` · ` separators, while empty `⚠️ LIMITES` and `🧭 SUITE` blocks are absent.
...
- `SSRP-022 visible closure editorial`: every completion claim includes `📰 ÉDITORIAL` with `updated`, `not impacted · <concrete reason>`, or `needs review · <surface>`; a material gap blocks closure, `No declared public surface` is valid evidence, and unaffected work creates no filler content.
