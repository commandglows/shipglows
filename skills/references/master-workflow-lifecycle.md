...
supersedes: []
evidence:
  - "Operator correction 2026-09-01: master workflows resolve delivery posture from canonical business context before branch selection and recover missing truth through one product question."
...
At project/chantier start, first-write, interrupted-resume, each coherent milestone, sensitive-operation, and closure boundaries of a Git-backed chantier, apply `git-persistence-preflight.md` and the standing convergence pass. This is silent when healthy: resolve canonical business `delivery_posture`, derive the integration branch, refresh/prune, inspect branch/upstream/PR/worktree relationships, and safely reconcile owned state. Missing posture asks one product question, persists that exact business field, and resumes; pitch, runtime live state, and agent instructions never substitute. Before auth, payment, permission, migration, destructive, tenant, secret, production, or private-data mutation, require the relevant baseline backed up remotely; never manufacture a checkpoint from incomplete, failing, secret-bearing, ambiguous, or unrelated work.
