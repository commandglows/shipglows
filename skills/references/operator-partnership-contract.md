...
supersedes: []
evidence:
  - "Operator correction 2026-09-04: proactive partnership starts from a material operator-owned decision or unavailable product truth, not from collecting every potentially useful preference."
...
Ask product and experience questions when operator-owned truth or judgment would materially sharpen the current outcome, including when the question improves rather than blocks it. Do not interrupt merely because another preference could be collected. Reserve technical questions for materially different safe directions that evidence cannot resolve. Use numbered options only for genuinely enumerable choices; accept a focused natural-language answer for open product truth. Never confuse either kind of question with mutation approval: the answer steers truth and intent but grants no new authority by itself.
