...
`context`, `contexte`, `env`, and `environment` are direct read-only modes. Load `agent-runtime-awareness.md`, resolve the execution envelope, read the global development-environment file, the selected managed surface's `ENVIRONMENT.md`, and the matching live registry entry, then report the host/agent/terminal/session/machine identity, active architecture, exact managed URL, Python version/`uv`/commands, Playwright Chromium installation path, MCP configuration and verification, relevant mobile and Windows toolchain state, its exact next action, and current-turn callable tools. Run `$SHIPGLOWS_ROOT/tools/project_git_policy.py` against the project and visibly report the effective task-branch and worktree creation policies plus their canonical source; missing or invalid values render independently as `forbidden` with the resolver reason. Always include the resolver guidance that `forbidden` prevents silent creation but can be changed after discussing a justified need with the user. For Flutter, also report the registry-backed active development target, managed session mode, logical `flutter run -d <device>` command and live state; current registry `status` outranks `flutterStartupState` and retained process fields, and merely available targets remain separate. Inspect direct and deferred/searchable tool catalogs, native transport reachability, and a safe probe before classifying capability; distinguish installed, configured, discovered, transport reachable, callable, failed, and not-exposed states. Contradiction or transport failure triggers targeted refresh before verdict. Never start a server, substitute a framework default port, describe configured Playwright or recorded Python as absent merely because its tool is missing from the first visible list, or give a vague restart instruction when the exact owning host can be identified.

List merely available targets separately.
