...
Security covers authn/authz and tenant boundaries, input/schema validation, secrets/config, HTTP/cookies/CORS/CSRF, data protection, logging/replay/source maps, webhooks, uploads, quotas and abuse. Reliability covers async failure, tests, health/diagnostics, deployment rollback and monitor/alert distinction. Deep dependency and performance work route to their own `010` modes. Do not mutate unless separately authorized.
