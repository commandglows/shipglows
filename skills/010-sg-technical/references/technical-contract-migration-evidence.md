...
| `310-sg-github-hygiene` | `github` | `references/github-hygiene-playbook.md` | Git sync and branch drift; stale refs; PR freshness; Dependabot configuration and queue; bounded safe fixes; terminal disposition ledger; item-scoped blockers; fresh GitHub truth after each queue mutation; explicit specialist routing and merge safety. |
...
`010-sg-technical` is the only active public technical identity. The five names in the matrix are source provenance only, never aliases, examples, runtime entries, public pages, redirects, wrappers, fallbacks, or source directories.
