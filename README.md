...
- support local access through SSH tunnel tooling, including password-first pairing promoted safely to a per-device SSH key; this flow is verified on a physical Android Termux device
...
The runtime menu exposes `s a` / **Authentication** to inspect redacted local
...
```bash
curl -fsSL https://shipglows.com/shipglows-script | SHIPGLOWS_INSTALL_MODE=local sh
curl -fsSL https://shipglows.com/shipglows-script | sudo env SHIPGLOWS_INSTALL_MODE=full sh
```
