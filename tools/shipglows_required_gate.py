...
try:
    from tools.project_delivery_policy import inspect_project as inspect_delivery_policy
except ModuleNotFoundError:  # Direct execution from the tools directory.
    from project_delivery_policy import inspect_project as inspect_delivery_policy
...
def _configured_contract(project: Path, config: Path) -> ProjectContract:
    ...
    delivery = inspect_delivery_policy(project)
    integration = delivery.integration_branch if delivery.state == "resolved" else None
    return ProjectContract(branch, lanes, str(config.relative_to(project).as_posix()), integration)
...
def inspect_project(project: Path) -> ProjectContract:
    ...
    return ProjectContract(
        _declared_production_branch(project), tuple(lanes), "detected", integration
    )
...
def render_workflow(contract: ProjectContract) -> str:
    branches = tuple(
        dict.fromkeys(
            branch
            for branch in (contract.production_branch, contract.integration_branch)
            if branch
        )
    )
    branch_list = ", ".join(_yaml_quote(branch) for branch in branches)
