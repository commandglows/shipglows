...
owner: "Diane"
user_story: "En tant qu'operatrice ShipGlowz, je veux suivre les checklists techniques de chaque projet par cycles et conserver leur progression et leur historique, afin de savoir exactement ou en est chaque projet sans transformer TASKS.md en checklist geante."
confidence: high
risk_level: high
security_impact: yes
docs_impact: yes
linked_systems:
  ...
  - "tui/"
depends_on:
  - artifact: "shipglowz_data/workflow/specs/shipflow-tdd-and-manual-checklist-artifacts.md"
...
🟢 [ShipGlowz] spec: Project Lifecycle Checklist Operating Model | status: ready | path: shipglowz_data/workflow/specs/project-lifecycle-checklist-operating-model.md | next: /102-sg-start project-lifecycle-checklist-operating-model
...
En tant qu'operatrice ShipGlowz, je veux suivre les checklists techniques de chaque projet par cycles et conserver leur progression et leur historique, afin de savoir exactement ou en est chaque projet sans transformer TASKS.md en checklist geante.

## Minimal Behavior Contract

ShipGlowz must let a governed project declare applicable lifecycle domains and instantiate reusable Markdown playbooks/checklists without replacing the existing local task tracker or chantier registry. A checklist item may be one-time, recurring, cyclic, or event-triggered; completed work must remain traceable, while open follow-ups route to the existing execution or editorial tracker. A reader must be able to derive current project state, domain progress, overdue or upcoming routines, today's actions, and next week's actions from the same versioned artifacts that skills consume. The easy-to-miss edge case is treating a recurring routine as permanently checked: each recurrence needs its own due state and evidence while the reusable definition remains stable.

## Success Behavior

- Preconditions: A project has a canonical `shipglowz_data/` governance root and declares the surfaces/domains that apply to it.
...
- Add parser/contract fixtures before changing the ShipGlowz app UI.
...
- No full ShipGlowz Flutter UI implementation in this first contract phase.
...
- Markdown and Git remain usable without the ShipGlowz app.
...
- Document contracts: `canonical-paths.md`, `task-registry-routing.md`, `operational-record-format.md`, playbook/checklist README contracts, and `shipflow-tdd-and-manual-checklist-artifacts.md`.
...
- Project/app context: the existing ShipGlowz app shell must be inspected only after the contract and parser fixture are stable.
...
- Downstream systems: `309-sg-tasks`, content/editorial routing, TUI dashboard, future ShipGlowz app, weekly review, and project status reporting.
...
- [ ] Task 3: Add pilot playbook/checklist pairs for publishability and ongoing technical operations.
  - File: `shipglowz_data/workflow/playbooks/`, `shipglowz_data/workflow/checklists/`
...
- [x] Task 4: Add a parser/status helper for lifecycle instances.
  ...
  - Notes: Reuse existing parser conventions where possible. **Implemented in `tools/shipglowz_project_lifecycle_status.py` with a deterministic Markdown fixture and focused tests; checklist-instance parsing remains.**
...
- [ ] Task 7: Prepare the ShipGlowz app handoff contract.
  - File: `shipglowz_data/technical/` and the app project after contract approval
  ...
  - Notes: This task does not authorize a broad UI rewrite. **Read model draft added in `shipglowz_data/technical/project-lifecycle-read-model.md`; app implementation remains out of scope.**
...
- Needed: yes
- Checklist path: `shipglowz_data/workflow/test-checklists/project-lifecycle-checklist-operating-model.md`
...
| 2026-07-28 00:00:00 UTC | 001-sg-build | GPT-5 Codex | Added the future ShipGlowz app read-model contract without starting a UI rewrite | partial | Run formal verification and manual lifecycle proof |
