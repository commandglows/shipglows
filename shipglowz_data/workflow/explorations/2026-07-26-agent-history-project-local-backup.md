...
linked_systems:
  ...
  - Codex CLI
evidence:
  - "cli/lib.sh: shipglowz_init_project prépare déjà des fichiers projet-locaux après clone"
...
- `cli/lib.sh` — contient le cycle de vie `deploy_github_project`, `shipglowz_init_project` et `env_remove`.
- `cli/config.sh` — centralise les réglages et constitue le bon emplacement pour le chemin et le mode de sauvegarde.
- `shipglowz_data/technical/context.md` — confirme que ShipGlowz est propriétaire du cycle de vie des environnements et du launcher Codex.
- `shipglowz_data/technical/runtime-cli.md` — indique les hotspots et les invariants de la CLI.
...
Le besoin est donc une **copie projet-locale de toutes les sessions**, synchronisée avec le dépôt distant. Ce n'est pas une sauvegarde centrale ShipGlowz et ce n'est pas seulement un symlink.

## Option Space

### Option A: collecteur ShipGlowz projet-local

- Summary: `shipglowz_init_project` crée `.agent-history/`; un collecteur copie les nouvelles sessions depuis les emplacements natifs vers ce dossier.
...
- Stockage uniquement dans `~/.local/share/shipglowz` — ne survit pas à la suppression du serveur.
