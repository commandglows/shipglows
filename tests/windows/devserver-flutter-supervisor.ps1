...
function Stop-SgFixtureProcess([Diagnostics.Process]$Process) {
    if (-not $Process) { return }
    try {
        if (-not $Process.HasExited) { $Process.Kill(); $Process.WaitForExit() }
    } catch [InvalidOperationException] {
        # The short-lived fixture may exit between HasExited and Kill.
...
$bulk=New-Object Diagnostics.Process;$bulk.StartInfo.FileName=$fixturePowerShell;$bulk.StartInfo.Arguments="-NoProfile -File `"$bulkScript`"";$bulk.StartInfo.UseShellExecute=$false;$bulk.StartInfo.RedirectStandardOutput=$true;$bulk.StartInfo.RedirectStandardError=$true
...
$killScript=Join-Path ([IO.Path]::GetTempPath()) ("sg-kill-{0}.ps1" -f [guid]::NewGuid().ToString('N'));[IO.File]::WriteAllText($killScript,'param([string]$EventName);for($i=0;$i -lt 100;$i++){[Console]::Out.WriteLine("k$i")};$signal=[Threading.EventWaitHandle]::OpenExisting($EventName);try{[void]$signal.Set()}finally{$signal.Dispose()};Start-Sleep 30')
$killEventName="Local\ShipGlowsDrain-$([guid]::NewGuid().ToString('N'))";$createdNew=$false;$killReady=New-Object Threading.EventWaitHandle($false,[Threading.EventResetMode]::ManualReset,$killEventName,[ref]$createdNew)
$killed=New-Object Diagnostics.Process;$killed.StartInfo.FileName=$fixturePowerShell;$killed.StartInfo.Arguments="-NoProfile -File `"$killScript`"";$killed.StartInfo.UseShellExecute=$false;$killed.StartInfo.RedirectStandardOutput=$true;$killed.StartInfo.RedirectStandardError=$true
$killed.StartInfo.Arguments="-NoProfile -File `"$killScript`" $killEventName"
try{[void]$killed.Start();$pump=New-SgFlutterStreamPump $killed;$oq=New-Object 'Collections.Concurrent.ConcurrentQueue[string]';$eq=New-Object 'Collections.Concurrent.ConcurrentQueue[string]';if(-not$killReady.WaitOne([TimeSpan]::FromSeconds(10))){throw 'Post-kill fixture did not finish writing before termination.'};$killed.Kill();$killed.WaitForExit();if(-not(Drain-SgFlutterStreams $pump $oq $eq 5)-or-not$pump.OutputEnded-or-not$pump.ErrorEnded-or$oq.Count-ne100){throw 'Post-kill drain lost buffered stdout or stream completion.'}}finally{if(-not$killed.HasExited){$killed.Kill();$killed.WaitForExit()};$killed.Dispose();$killReady.Dispose();Remove-Item -LiteralPath $killScript -Force -ErrorAction SilentlyContinue}
...
$continuous=New-Object Diagnostics.Process;$continuous.StartInfo.FileName=$fixturePowerShell;$continuous.StartInfo.Arguments="-NoProfile -File `"$continuousScript`"";$continuous.StartInfo.UseShellExecute=$false;$continuous.StartInfo.RedirectStandardOutput=$true;$continuous.StartInfo.RedirectStandardError=$true
...
try {
    ...
    $reload=New-Object Diagnostics.Process;$reload.StartInfo.FileName=$fixturePowerShell;$reload.StartInfo.Arguments="-NoProfile -File `"$reloadScript`"";$reload.StartInfo.UseShellExecute=$false;$reload.StartInfo.RedirectStandardInput=$true;$reload.StartInfo.RedirectStandardOutput=$true;$reload.StartInfo.RedirectStandardError=$true
