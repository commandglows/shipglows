...
try {
    ...
    foreach ($required in @('exact existing local branch','cannot overwrite, discard, or relocate current changes','exact branch availability','exact path availability','resolved base','current worktree remains untouched','any fast criterion is missing, uncertain, or false','new target, effect, or risk','prior approval is invalid','ordinary push','execute directly','force push retains all stricter','MAP-SMALL-CHANGE','execute it from the operator''s exact request','without an approval prompt','becomes a chantier')) {
    ...
    foreach ($required in @('clear bounded request','few coherent enumerable actions and targets','material direction','does not authorize a chantier','without another prompt','targeted file modification','exact-scope commit','ordinary resolved push','Local versus remote and model reasoning effort never change this classification','🧭 VALIDATION RAPIDE','🧭 PLAN À VALIDER','Force push and destructive or irreversible actions retain stricter dedicated gates')) {
