...
try {
    ...
    foreach ($required in @('🧭 PLAN À VALIDER','🧭 VALIDATION RAPIDE','Objectif','Périmètre','Actions','Preuves','one or two sentences','exact action','exact target','main safety guarantee','Exact micro-request authority','does not create or approve a chantier','material change','MAP-EXACT-MICRO-REQUEST','MAP-MICRO-TO-CHANTIER','MAP-FAST-SWITCH','MAP-FAST-WORKTREE','MAP-FAST-INELIGIBLE','MAP-FAST-REPLACEMENT','MAP-REMOTE-PUSH','`git push` always requires the full plan')) {
        if ($approvalContract -notmatch [regex]::Escape($required)) { throw "Mutation approval contract is missing: $required" }
    }
    foreach ($required in @('exact existing local branch','cannot overwrite, discard, or relocate current changes','exact branch availability','exact path availability','resolved base','current worktree remains untouched','any fast criterion is missing, uncertain, or false','new target, effect, or risk','prior approval is invalid','every `git push` uses the full','force push also retains all stricter','MAP-SMALL-CHANGE','execute it from the operator''s exact request','without an approval prompt','becomes a chantier')) {
    ...
    foreach ($required in @('exact micro-request','does not authorize a chantier','without another prompt','local-only','readily reversible','cannot overwrite, discard, delete, force, publish, deploy, message, change credentials/permissions, start a process, or affect unrelated changes','🧭 VALIDATION RAPIDE','🧭 PLAN À VALIDER','Ordinary exact-scope local commits never need a separate prompt','`git push` always uses the full plan')) {
