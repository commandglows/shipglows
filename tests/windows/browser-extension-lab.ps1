...
try {
    $static = Join-Path $fixture 'static'; $built = Join-Path $fixture 'built'; $ambiguous = Join-Path $fixture 'ambiguous'
    foreach ($path in @($static,$built,$ambiguous)) { New-Item -ItemType Directory -Path $path -Force | Out-Null }
