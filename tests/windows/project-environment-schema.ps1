...
'@
    ...
    if ($environment.Schema -ne 'shipglows-project-environment/v2' -or $environment.Port -ne 3002) { throw 'Legacy v0 was not migrated to the exact v2 schema.' }
    ...
    if ([regex]::Matches($content, [regex]::Escape('- Environment schema: `shipglows-project-environment/v2`')).Count -ne 1) { throw 'The v2 schema marker is missing or duplicated.' }

    $hash = (Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash
    [void](Write-SgProjectEnvironment $project 3002)
    if ((Get-FileHash -LiteralPath $path -Algorithm SHA256).Hash -ne $hash) { throw 'The v2 rewrite is not byte-idempotent.' }
    ...
    $v1Project = Join-Path $fixture 'v1'
    New-Item -ItemType Directory -Path $v1Project -Force | Out-Null
    $v1Path = Join-Path $v1Project 'ENVIRONMENT.md'
    $v1 = "<!-- >>> ShipGlows development environment >>> -->`n## ShipGlows development environment`n`n- Environment schema: ``shipglows-project-environment/v1```n- Server manager: ``shipglows-devserver```n- Assigned port: ``3004```n- Canonical local URL: ``http://127.0.0.1:3004```n- Live status authority: Windows ShipGlows DevServer registry`n<!-- <<< ShipGlows development environment <<< -->`n"
    [IO.File]::WriteAllText($v1Path, $v1, [Text.UTF8Encoding]::new($false))
    [void](Write-SgProjectEnvironment $v1Project 3004)
    if ((Get-SgProjectEnvironment $v1Project).Schema -ne 'shipglows-project-environment/v2') { throw 'Schema v1 was not migrated to v2.' }

    foreach ($invalid in @(
        [pscustomobject]@{ Name='future'; Content="# Notes`n`n<!-- >>> ShipGlows development environment >>> -->`n## ShipGlows development environment`n`n- Environment schema: ``shipglows-project-environment/v3```n- Server manager: ``shipglows-devserver```n<!-- <<< ShipGlows development environment <<< -->`n"; Error='Unsupported ShipGlows project environment schema' },
