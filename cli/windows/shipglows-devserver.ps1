...
function Get-SgUpdateSource {
    ...
    if ($dirty.Count -gt 0 -and -not $AllowDirty) {
        throw "The linked ShipGlows checkout has uncommitted changes, so the update stopped to preserve them. Inspect them with: git -C `"$root`" status --short. Commit or deliberately resolve those changes, then retry 'shipglows update'. No files were stashed or replaced."
    }
...
function Invoke-SgSkillsUpdate {
    ...
    if ($source.Channel -ne 'linked') {
        throw "Skills-only update requires the linked maintainer channel. Run 'shipglows update status' to inspect the active channel; use 'shipglows update' for the stable installed runtime."
    }
...
function Invoke-SgDeveloperToolsUpdate {
    ...
    if (-not (Test-Path -LiteralPath $installer -PathType Leaf)) { throw 'The installed ShipGlows developer-tool convergence engine is unavailable; run shipglows update first.' }
