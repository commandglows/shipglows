...
When a conversation needs the capabilities actually exposed by the Windows CLI, read the bounded snapshot at `%LOCALAPPDATA%\ShipGlows\DevServer\cli-capabilities.v1.json` (or `SHIPGLOWS_CLI_CAPABILITIES_FILE` when explicitly configured). Treat missing, invalid, future, or stale evidence as unavailable; reading this file must never execute the CLI.
