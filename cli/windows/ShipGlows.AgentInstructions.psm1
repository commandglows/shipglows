...
Before capability or runtime claims, identify the current execution envelope: operating system, agent surface, terminal host, local/remote session location, and physical/virtual/container machine kind. A Codex CLI process launched through Node from Rio or another terminal is not Codex Desktop.
When a conversation needs the capabilities actually exposed by the Windows CLI, read the bounded snapshot at `%LOCALAPPDATA%\ShipGlows\DevServer\cli-capabilities.v1.json` (or the explicit absolute `SHIPGLOWS_CLI_CAPABILITIES_FILE` override); reading it must never execute the CLI.
For a ShipGlows-managed project, read the selected managed surface's `ENVIRONMENT.md` for its durable assigned URL and the matching ShipGlows DevServer registry entry for live status. Registry `status` is authoritative; `flutterStartupState`, retained PIDs, and earlier observations never override `stopped` or `error`. For Flutter, report the registry-backed active development target, managed session mode, logical `flutter run -d <device>` command, and state; keep other installed or repository targets classified as available rather than active.
...
For an Obsidian plugin, do not invent a localhost URL, discover or select a personal vault, or treat build success as host validation. Review the project's explicit build command when artifacts are required; use only an explicitly declared development vault for `s start`, and use `s obsidian-lab -ProjectPath <path> -Headless -Json` for disposable isolated load proof of an already-built approved plugin.
The Obsidian Lab separates profile and vault data but is not an OS sandbox, and local BRAT artifact conformity never authorizes release or publication.
...
Keep plugin installation, skill discovery, tool exposure, native transport reachability, and a successful safe probe separate. A standalone Codex CLI can expose Computer Use and Node REPL without the Codex Desktop native pipe. In that case restarting the same CLI cannot repair the transport: route GUI-dependent work to Codex Desktop or another suitable proof surface. On contradiction or transport failure, refresh only the execution envelope, selected managed surface, matching registry entry, and affected transport before verdict; exhaust safe read-only diagnosis and never hand the operator a vague "restart Codex" or "reactivate the connector" instruction.
...
For ordinary Flutter implementation and debugging, use the managed live `flutter run` session for the selected target so logs and reload remain attached. On Android, honor an explicit connected device; otherwise reuse a ready device or start the provisioned `ShipGlows_API_36` emulator and wait for readiness. Reserve standalone builds for releases or explicit package-sensitive checks such as native plugins, DLLs, installation, production-mode behavior, performance, or startup without Flutter attached.
After any successful local Windows release or Android APK build, do not stop at build success. Register the validated artifact with `%USERPROFILE%\.shipglows\runtime\cli\windows\shipglows-build-artifacts.ps1 register-local`, keep Local and CI lanes distinct, and report the refreshed official Desktop shortcut. Never substitute a manual shortcut that points into a worktree.
