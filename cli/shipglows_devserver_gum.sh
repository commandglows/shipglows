#!/bin/bash
# ShipGlows — Gum-styled menu with instant keyboard shortcuts
# Sourced by shipglows.sh when gum is available
#
...
run_menu() {
    while true; do
        ...
        _gum_run_menu "ShipGlows DevServer" "" "screen" "${MAIN_MENU_ITEMS[@]}"
        local rc=$?
        if ui_should_return_to_main_menu; then
            continue
        fi
